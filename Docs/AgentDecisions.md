...
后果：第一次 cache miss 不再因 visual projection 独占 period 控件、滚动或导航；SwiftData 与 CloudKit 的线程/隔离边界保持完整。输入投影会增加一次有界复制，core assembly 仍会有受预算约束的短 main-actor 段，不能夸大成“全 snapshot 已后台化”。R1 完成后停止主动重构，真实 Release/Archive trace 留作 release gate。
