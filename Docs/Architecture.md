...
Protected Health samples live in a separate `AppleHealthReplicaSchemaV1` SwiftData store with CloudKit disabled. `AppleHealthReplicaRepository` is its sole persistence owner; `AppleHealthReplicaSyncService` supplies saved workout/sleep anchors to the HealthKit change reader and commits both streams' upserts, explicit deletions, anchors, and successful-sync metadata only after both queries succeed. The main V14 registry, `SyncDataSnapshot`, Cloud conflict merge/fingerprint, and restore paths never contain replica models. Timeline and Health task analytics refresh the replica, then read immutable interval snapshots. Hiding the timeline keeps the replica; Clear All removes its records and anchors.
