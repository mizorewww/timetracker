...
The current SwiftData schema is V10 (`1.9.0`). V9 removed the persisted `DailySummary` derived cache through a lightweight V8→V9 migration. V10 adds optional opaque Inbox suggestion identity fields; the V9→V10 migration deterministically initializes legacy rows and preserves the old “generated with no active suggestion” dismissal state. Legacy V9 Inbox model shapes remain frozen for migration, and current analytics still creates disposable `DailySummarySnapshot` values from ledger facts.
