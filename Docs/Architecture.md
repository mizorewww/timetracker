...
Ordinary scene mutations enqueue Widget, Watch, and Live Activity publication through one shared `CommittedMutationSystemProjectionScheduler` per physical `TimerStoreScope`. Publication is not part of the mutation caller's completion path: a worker opens a fresh read context, materializes immutable projection DTOs once per generation, and isolates each sink's failure. Work that has started is allowed to finish; only pending generations are coalesced, receipt bookkeeping is bounded, and an oversized pending event set safely degrades to `.fullSync`. A queued next-MainActor-turn `StoreMutationBroadcaster` independently converges sibling scenes as read-only consumers, so they do not repeat sync recording, system publication, or suggestion work. Every durable SwiftData transaction has an explicit stable history author at its outer save boundary: scene and store-scoped business mutations use `localMutation`, sync snapshot restore uses `syncReconciliation`, and startup migration/seed work uses `bootstrapMaintenance`. A missing or unknown author is never evidence of a local mutation. The history driver converts type-erased changed entity names into conservative ID-free domain events; an unknown future entity becomes `.fullSync`, so a worker never acknowledges a frontier after skipping a model it does not understand. The sync recovery snapshot remains a separate synchronous post-commit durability boundary until persistent-history recovery replaces that dependency.
...
Protected Health samples live in a separate `AppleHealthReplicaSchemaV1` SwiftData store with CloudKit disabled. `AppleHealthReplicaRepository` is its sole persistence owner; `AppleHealthReplicaSyncService` supplies saved workout/sleep anchors to the HealthKit change reader and commits both streams' upserts, explicit deletions, anchors, and successful-sync metadata only after both queries succeed. The repository preserves HealthKit point samples whose start and end dates are equal so their anchors can advance; reverse intervals remain invalid and roll back the whole generation. Duration-based timeline and analytics projections ignore zero-width intervals without discarding the source fact. The main V14 registry, `SyncDataSnapshot`, Cloud conflict merge/fingerprint, and restore paths never contain replica models. Timeline and Health task analytics refresh the replica, then read immutable interval snapshots. Hiding the timeline keeps the replica; Clear All removes its records and anchors.
...
Task-editor checklist deletion is a draft mutation keyed by the row's stable UUID, not a direct View write or a captured source index. The shared row exposes it through a native secondary `Menu`; iOS/iPadOS additionally use a trailing `swipeActions` button with full-swipe disabled, while macOS adds the same command to the row context menu. Saving continues through task-editor replacement and `ChecklistDraftService`, so removed persisted rows receive the existing tombstone/visual cleanup semantics and stale baselines remain rejected.
...
A physical Cloud recovery reset invalidates post-commit history work before removing the store. Under the store mutation lock and durable-root file lock it increments a retained reset epoch, then removes SQLite/WAL/SHM plus every lane cursor and full-reconciliation attempt while preserving the mutation lock and epoch sidecar. Projection coordinators bind the physical Core Data store UUID and reset epoch only when epoch reads immediately before and after metadata resolution agree, then revalidate that epoch at begin and acknowledgement. Work suspended across a reset therefore cannot publish a stale history frontier into the replacement store. A dedicated actor pages chronological SwiftData history through an isolated `@ModelActor`, captures the tail before each effect, and advances each lane only after its sink succeeds; an expired frontier falls back to a full reconciliation instead of deleting shared history.
...
Finalize produces a deterministic baseline-to-overlay diff, not facts. `AITaskWorkspacePlanGeneratorViews` presents a read-only review with create/update/archive/delete/reuse counts, full Task or Checklist context, and before→after values. The primary action is `Apply N Changes`; Category/Checklist deletion, Task archival, and other destructive impact require a native destructive confirmation. Generation cancellation preserves the request. Provider/tool failures and apply conflicts preserve the preview where one exists; the feature never falls back to create-only JSON or lets the model write SwiftData.
...
The macOS app root separately owns one application-level `MacKeyboardShortcutSettings` observable and injects that same instance into the main scene, Settings scene, and `TimeTrackerCommands`. Its durable command writes one size-limited atomic Codable blob through `AppDefaults.shared`; the read path falls back without rewriting corrupt, oversized, unsupported, duplicate, or reserved payloads. Shortcut changes increment a revision so SwiftUI reconstructs the affected menu items immediately. This state is intentionally outside `TimeTrackerStore`, SwiftData, CloudKit, scene focus, and the `KeyboardShortcuts.Name` global-hotkey storage path.
