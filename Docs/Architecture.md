...
A physical Cloud recovery reset invalidates post-commit history work before removing the store. Under the store mutation lock and durable-root file lock it increments a retained reset epoch, then removes SQLite/WAL/SHM plus every lane cursor and full-reconciliation attempt while preserving the mutation lock and epoch sidecar. A malformed or oversized epoch remains fail-closed for ordinary readers; the explicit reset boundary alone may repair it by invalidating all lane metadata, quarantining the corrupt fence, and publishing a fresh nonzero epoch. Projection coordinators bind the physical Core Data store UUID and reset epoch only when epoch reads immediately before and after metadata resolution agree, then revalidate that epoch at driver begin and acknowledgement. The scheduler separately checks its container revision around materialization, cache, publication, and acknowledgement so an observed replacement drops stale DTOs; framework publication is not atomically excluded from a concurrent replacement. The hard guarantee is that work suspended across reset or replacement cannot acknowledge a frontier for the replacement store. Each of the four lanes owns its own cursor, failure, retry, and full-reconciliation attempt; sibling success never acknowledges a failed lane. A malformed or oversized attempt invalidates its lane's older cursor before being quarantined, so its absence cannot expose an obsolete incremental frontier. The driver captures the history tail before each effect, and an expired frontier falls back to a full reconciliation instead of deleting shared history.
