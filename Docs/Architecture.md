...
A physical Cloud recovery reset invalidates post-commit history work before removing the store. Under the store mutation lock and durable-root file lock it increments a retained reset epoch, then removes SQLite/WAL/SHM plus every lane cursor and full-reconciliation attempt while preserving the mutation lock and epoch sidecar. Projection coordinators bind the physical Core Data store UUID and reset epoch only when epoch reads immediately before and after metadata resolution agree, then revalidate that epoch and the scheduler's container revision before materialization, publication, and acknowledgement. Work suspended across a reset or container replacement therefore cannot publish or acknowledge a stale frontier. Each of the four lanes owns its own cursor, failure, retry, and full-reconciliation attempt; sibling success never acknowledges a failed lane. The driver captures the history tail before each effect, and an expired frontier falls back to a full reconciliation instead of deleting shared history.
