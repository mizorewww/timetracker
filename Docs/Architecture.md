...
AI task-plan generation is explicit and draft-first. `LLMTaskPlanService` sends only the user's 4 KiB request, the sanitized 4 KiB planning instructions, and icon/color allowlists under an immutable system contract. Generation streams over server-sent events (`stream: true` with `stream_options.include_usage`): the sheet shows a live output-token estimate (provider-reported usage wins over the character heuristic) so a stalled model is visible, and the accumulated `content` still passes the same post-completion validation — one flat response capped at 128 KiB, textual references mapped to locally generated UUIDs, duplicate/orphan references, cycles, child-task categories, depth beyond six, invalid fields, more than 16 categories, 128 tasks, 256 checklist items per task, or 1,024 checklist items total all rejected. The provider's reasoning trace (`reasoning_content`) and the raw JSON answer ride on the draft as display-only provenance, viewable in the preview's Reasoning/Raw Output disclosures and never persisted as facts. A buffered non-streaming path remains for tests and callers without SSE. The SwiftUI sheet owns the mutable preview; it can rename or remove proposed items but cannot write facts directly.
