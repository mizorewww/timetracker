...
`SyncConflictService.prompt()` 是 throwing read boundary：只有合法 state 中确实没有完整 pending conflict 才返回 `nil`。损坏、超限、权限和文件系统错误必须传播给 Store；禁止用 `try?` 把它们转换成“无冲突”。已经写入 local snapshot 的 `recordLocalMutation` 会在同一 state lock 内返回 `.recorded(prompt:)`，调用方不得为同一次 mutation 再读一次 `prompt()`；`.notRecorded` 则保留原本的 throwing read 边界。persistent-history 等 at-least-once consumer 重放相同 snapshot 时，`recordLocalMutation` 必须在既有 store/state 锁序内比较完整运行时 state：所有后置条件均已满足就仍返回 `.recorded`，但不得推进 generation 或重写 manifest/slot。不能只比较 fingerprint，因为 pending conflict 的 working snapshot 与受保护 local branch 可以不同。Watch 等已经提交业务 mutation 的入口把真正随后发生的 prompt 读取失败报告为 post-commit refresh failure，不能把 terminal command 结果倒写成未提交。
