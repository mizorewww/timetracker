...
`TimerStoreScope`、`StoreScopedTimerMutationLock` 与 `StoreScopedTimerMutationTransaction` 是协调器的事务底座。持久 store 使用解析既存祖先符号链接后的 canonical 文件 URL，同一内存 container 在完整生命周期内复用一个显式 UUID；锁文件与 store 同目录并使用 `.timer-mutations.lock` 后缀。锁实现复用 `PathFileLockRegistry`/`PathProcessFileLock` 的进程内递归锁与跨进程 `flock`，不得另造第二套文件锁。transaction 必须先取得 store lock，再创建新的 `ModelContext`、关闭 autosave，并用一次 `performAtomicMutation` 统一保存或回滚。锁内只做授权、fresh fetch、plan、model mutation 和 commit；网络、UI 等待及 post-commit read-model/system-surface refresh 留在锁外。普通 Timer 的 `allowParallelTimers` 必须由 `TimerAdmissionPreferenceResolver` 在此 fresh context 内解析，Watch、App Intent 和 facade 不得传递缓存 Bool；Watch processor 同时把锁内实际 outcome 的 events 交给 post-commit surfaces/sync，不能从旧 Watch snapshot 猜测会停止哪些 segment。Timer、Pomodoro、segment 编辑、task lifecycle/draft、countdown update/delete、同步快照、checklist quick commands 和 Inbox primary commands 已共用此锁域；新增写入口不得退回 scene-owned `ModelContext`。
