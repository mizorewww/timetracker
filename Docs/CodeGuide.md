...
- `LedgerStore+RecordIndexes.swift` 为每个任务维护按 startedAt/UUID 完全决胜的最近 8 条 segment ID，并在 scoped replacement 时增量更新；删除或改期会从该任务完整 ID set 补齐被挤出的第 9 条。Task Detail 的分钟级刷新只从 day index 读取当前与上一比较周期，再从 recent index 合并任务分支的最近 8 条；不得重新读取该分支全部历史。若 CloudKit 关系隔离使有界候选不可见，facade 才为正确性回退到完整 task index，正常一致数据不走该路径。
