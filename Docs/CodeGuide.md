...
Inbox 拖拽重排由 `StoreScopedInboxCommandCoordinator` 处理。Scene 先从当前完整 open item 集合建立 `InboxOrderMutationBaseline`，同时固化确定性原顺序和每项 `clientMutationID`；协调器取得共享 store lock 后用 fresh context 重新解析逻辑 winner，并要求 baseline、目标 ID 集合和数量全部精确一致。新增、完成、删除、编辑、远端 restore、已有 reorder，甚至未正确推进 mutation ID 的 sort 变化都会拒绝旧拖拽；拒绝只刷新 Inbox read model，不写 sort、文本或同步 generation。成功重排仍复用 Inbox logical-sibling 规则。add/toggle/title/delete/suggestion writer 尚未因此自动获得相同跨场景保证，后续迁移时不得误报为已覆盖。
