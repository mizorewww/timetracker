...
- Typography presentation：macOS 的正文、主 identity、用户决策所需说明、状态/
  警告和有文字的操作使用系统 `body/callout`；path、timestamp、badge、count、chart
  axis/range 与 debug/provider/build 信息才保留 `subheadline/caption/footnote`。
  跨平台 View 在自己的 feature/component 文件内以局部 semantic property 映射
  macOS 与 iOS 样式，不能在 root 注入全局 font、硬编码 point size、缩放或建立
  typography framework。`testUIRefactorBaselineScreenshots()` 是 Today、Inbox、
...
- macOS：单实例主 `Window` 承载 `NavigationSplitView` 工作区；独立系统 Settings scene、主窗口和 Settings 共享一个应用级 `TimeTrackerStore`，避免复制 CloudKit observers、自动 AI 建议与系统表面同步。应用根另持有一个设备本地 `MacKeyboardShortcutSettings`，注入主场景、Settings 和 `TimeTrackerCommands`；设置使用 `KeyboardShortcuts` 的 binding recorder，菜单继续使用原生 SwiftUI `Commands`。`Command-N`、`Command-,`、`Command-1...5` 固定，自定义写入 `AppDefaults.shared` 的单个原子 blob，不进入 `TimeTrackerStore`、SwiftData/CloudKit 或库的全局 hotkey 存储。
...
- Task category headers：`TaskCategorySectionHeaderStyle` 明确共享分类 header 的三个展示意图。Tasks 管理页使用 `.standard`，在 macOS 为 `body.semibold`、iOS/iPadOS 保留 `subheadline.semibold`，并保留完整 action/辅助字号行为；临时层级选择器显式使用 `.compact` 的 caption 密度；iPad/macOS 持久侧边栏显式使用 `.sidebar` 的 `body.semibold`，且 SF Symbol 继承同一系统字体。调用方不得用 Bool 或硬编码 point size 再次混淆 picker 与 sidebar。
