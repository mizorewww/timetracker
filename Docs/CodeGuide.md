...
- Checklist editor presentation/mutation：`ChecklistEditorRow` 是任务编辑器 checklist 行的布局所有者。它使用原生垂直轴 `TextField`，任务标题传入无界 `lineLimit`、以文本的完整 intrinsic height 扩张；完成控件、symbol picker 与 More menu 围绕整个多行文本块居中，并分别保留 44 pt iOS/iPadOS、28 pt macOS 的最小操作目标。`ChecklistTitleTextField` 的有界参数只供需要紧凑策略的调用方使用，Inbox 继续保留 `1...5`，不得把该上限重新施加到任务 checklist。删除按稳定 UUID 回调到 `TaskEditorSession.deleteChecklistItem(id:)`，不能在 View 中按暂存数组下标直接删除。每行常显 `Menu` 作为二级操作；iOS/iPadOS 使用 trailing `swipeActions(allowsFullSwipe: false)`，macOS 使用 `contextMenu`，三者共享同一个草稿删除闭包。删除当前聚焦行时先清 focus；最终持久化仍由 editor save、`ChecklistDraftService` 与既有 tombstone/visual cleanup 负责，不另建 repository 写路径。
