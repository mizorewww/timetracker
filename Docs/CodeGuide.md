...
Cloud recovery 的物理 store reset 在既有 store-scoped mutation lock 内再持有 `DurableLocalFile` root lock，先原子递增保留的 reset epoch，再删除 SQLite、WAL/SHM、四 lane cursor 与 attempt。普通 cursor 调用方通过 `PersistentHistoryLaneCursorStore.registeringCurrentEpoch` 固化注册时 epoch；history driver 还必须按 `epoch-before → Core Data metadata store UUID → epoch-after` 注册，并且只在两次 epoch 相同时一起缓存 UUID/frontier store，避免把被替换 SQLite 的 UUID 绑定到新 epoch。禁止假定初始 epoch 为 0，也不得把 root lock、metadata 或文件 I/O 放在 MainActor，因为这些操作可能等待。begin、incremental CAS 与 full-reconciliation finalize 都重验当前 epoch；任何失败都丢弃 driver 缓存的注册，确保下一次 run 会重新绑定当前 epoch。因而 reset 前挂起的 worker 即使随后恢复，也不能重建旧 cursor；新 container 只能在新 epoch 下重新建立 baseline。reset epoch sidecar 与 mutation lock 本身不随 store basename 清理。
