...
- Analytics loading presentation：landing/detail 的 `AnalyticsLoadedSnapshot` 必须把 snapshot 与 request 原子发布，`AnalyticsSnapshotPresentationPhase` 统一首屏、同周期刷新与跨周期遮蔽状态。精确 evaluation cache 命中可同步呈现；跨周期冷缓存保留 section 壳但 redacted、禁用并隐藏旧数据语义。refresh indicator 保持固定布局槽，不能条件改变 `ViewThatFits` 的理想尺寸。
...
- Typography presentation：macOS 的正文、主 identity、用户决策所需说明、状态/
  警告和有文字的操作在所有主 App 平台共享系统 `body/callout`；path、timestamp、
  badge、count、chart axis/range 与 debug/provider/build 信息才保留
  `subheadline/caption/footnote`。跨平台 View 不得按 `os(...)` 映射同一产品角色的
  字号；密度差异由明确的 component style 或 compact/regular shell 表达。不能在
  root 注入全局 font、硬编码 point size、缩放或建立 typography framework。
  `testUIRefactorBaselineScreenshots()` 是 Today、Inbox、
  Tasks、Task Detail、Focus、Analytics、Settings 的确定性普通字号矩阵。
...
- Checklist editor presentation/mutation：`ChecklistEditorRow` 是任务编辑器 checklist 行的布局所有者。它使用原生垂直轴 `TextField`，任务标题传入无界 `lineLimit`、以文本的完整 intrinsic height 扩张；完成控件与 symbol picker 围绕整个多行文本块居中，并分别保留 44 pt iOS/iPadOS、28 pt macOS 的最小操作目标。`ChecklistTitleTextField` 的有界参数只供需要紧凑策略的调用方使用，Inbox 继续保留 `1...5`，不得把该上限重新施加到任务 checklist。删除按稳定 UUID 回调到 `TaskEditorSession.deleteChecklistItem(id:)`，不能在 View 中按暂存数组下标直接删除。行内不常显 More、垃圾桶或上下按钮；iOS/iPadOS 使用 leading `swipeActions(allowsFullSwipe: false)`，全平台使用只含 Delete 的 `contextMenu`，共享同一个草稿删除闭包。删除当前聚焦行时先清 focus；最终持久化仍由 editor save、`ChecklistDraftService` 与既有 tombstone/visual cleanup 负责，不另建 repository 写路径。Checklist、Category sorter 与 Quick Start pinned section 都由原生 `ForEach.onMove` 拥有排序，不复制 chevron 按钮；Quick Start 必须通过可见稳定 ID 把 offset 映射回完整 selection，不能把过滤后数组的 offset 直接写入未过滤数组。
...
- Facade lifecycle：`TimeTrackerStore+Configuration.swift` 负责首次配置与启动 projection catch-up；`TimeTrackerStore+Lifecycle.swift` 负责 mutation 边界和当前 scene read-model refresh；`TimeTrackerStore+RefreshLifecycle.swift` 负责启动/前台恢复触发，`TimeTrackerStore+SyncRefreshPipeline.swift` 负责 remote-import 恢复触发，`TimeTrackerStore+SyncObservers.swift` 接收进程内 durable-mutation notification 并只刷新当前 scene read models。`SystemActionPostCommitEffects` 是 App Intent 唯一的后置编排点：它只广播 exact committed `StoreDomainEvent` 并 enqueue store-scoped scheduler，不配置 facade repositories、生成临时 read model、等待 projection 或启动完整 lifecycle/自动 LLM。scene catch-up 不得重复 sync snapshot 或 system surface。
...
- Today：iPhone 使用 `List`，顺序为 Now、Overview、Weekly Time、Activity Heatmaps、Quick Start、Timeline、Forecast、Countdown。Overview 与两种可视化标题统一由 `HomeSectionHeader` 拥有：桌面卡片使用系统 `.headline`，iPhone `Section` 保留系统 header 字体，汇总值固定为 secondary monospaced caption；具体图表不得再复制标题样式。宽屏当前状态行中的 Now 也使用 card `HomeSectionHeader`；card header 统一以 `AppLayout.minimumInteractiveTarget` 作为最小高度，使有无 trailing action 的可见标题和首张卡片顶部保持一致。`DesktopTodayCurrentStateSections` 用原生 `HStack` 的 ideal height 和子 section 的 `maxHeight` 形成等高行：较高的 Now 决定行高，Overview card 填充到同一底边并在可用高度中居中指标；禁止用测量后的固定/最小高度、透明按钮、magic offset、magic padding 或设备分支伪造对齐。`.listSection` 不预留该槽位，iPhone 单列布局不参与宽屏等高契约。iPhone 可视化通过 `homeVisualizationListCard` 保留每图一张独立圆角卡片：系统 section row 必须保持透明，独立自绘背景从原生 content column 向两侧扩展一个 `AppLayout.cardPadding`，使外缘与 Overview 的 inset-grouped row 对齐，而图表内容只保留系统已经提供的一层 16 pt inset；不得恢复 iOS 26+ 会合并连续 section 的纯原生结构，也不得嵌套共享外卡。iPad/macOS 共享 `TodayHomeContent`；详情 viewport 扣除两侧 page padding、再受 1180 pt 上限约束后才得到实际内容宽度。宽屏 Timeline 与 Countdown 的无界行集合必须保留在 `LazyVStack`，避免 iPad/macOS 一次挂载完整数据集与辅助功能树。Weekly Time 与 Activity Heatmaps 组成 leading-aligned 的可视化组。周图用同一零基线的 positioned grouped bars 比较每天 Gross（固定左侧、系统绿）与 Wall（固定右侧、系统蓝），底部原生图例和逐 mark VoiceOver 值提供颜色以外的语义；不得堆叠二者或改动 Analytics 专用的 Wall 柱 + Gross 线模式。`ActivityHeatmapGrid` 是唯一 viewport 测量 owner，`ActivityHeatmapLayoutPolicy` 用实际宽度与 5/14/27/53 周选择 12...24 pt 整数 tile 和对应 2/3/4 pt 间距；fit 时禁用横向滚动并靠 leading，达到上限后不再制造巨型 tile，overflow 时不低于 12 pt 且保留最新日期初始锚点。月份标签由 Charts collision resolution 去重，range/legend 留在 scroll viewport 外；禁止恢复 phone/regular 固定尺寸表、读取屏幕宽度或逐 cell 建 AX node。实际内容宽度不足 1000 pt 时，后续 section 按语义顺序单列；达到 1000 pt 时，可视化组使用 678...748 pt 主列，Quick Start 使用剩余 300...410 pt 尾列。下一行由 Timeline 占弹性主列；存在 Forecast/Countdown 时，它们使用固定 360 pt 尾列。`HomeLayoutPolicy` 是这些断点和列宽的唯一 owner；辅助内容出现与否不得改变可视化列宽，不得另建 masonry、三列或第三方布局分支。Today 只有一个当前计时入口：无活动计时时为 Start Timer；有活动计时时根据并行偏好显示 Start Another Timer 或 Switch Timer。三种状态全部由 `TimerPickerMode` 驱动同一个 `HomeTimerPickerButton`，沿用 Start Another Timer 的原生 leading 整行样式，只改变准确的标题和图标；空 Quick Start 也复用该入口，不得复制突出按钮或另一套状态判断。这个主动作在 Now 内容流中保留可见文字。`TimerPickerCommandPolicy` 以 `.start`、`.startAnother`、`.switchTimer` 固化入口模式，并把任务选择命令限定为开始、切换或 `alreadyRunning`。计时选择器每行都由只读任务摘要与独立的尾部动作组成；Start、Switch 和 Stop 固定使用同一 icon-only 槽位。运行中任务进入独立 Running 区域，但一行只出现明确的 Stop，不再重复 Running badge；整行和选择摘要不得根据运行状态暗中改成停止命令。只有成功开始或切换后才关闭选择器，Stop 与失败都保留当前 picker 上下文。通用新建任务只存在于任务域和任务选择器，不与计时主操作竞争。
- Today Timeline rows：`TodayTimelineEntryRow` 只负责 tracked/Health 的数据与动作差异，二者都把时间、任务身份、来源和 duration 投影到 `TodayTimelineRecordContent`。Health 使用已计数的 `durationSeconds` 并保持只读；普通记录保留 live/fixed duration 与编辑菜单。不得在 entry wrapper 中复制 record 几何或按首行增加布局补丁。
- Today Timeline chart：`TimelineChartVerticalLayout` 可按真实本地化 skipped 胶囊宽度扩展纵向 axis gutter，`TimelineChartLayout.verticalAxisTicks` 负责沿时间轴过滤与胶囊冲突的 interior tick；`TimelineChartGrid` 中剩余时间文字始终使用 gutter 的语义 leading 基准。不得按 lane 数、gap 文案宽度或 tick role 增加水平 offset，也不得把动态 gutter 的 trailing edge 当作时间列锚点。
...
- Task identity and summary：脱离树结构的任务行或按钮统一从 `TaskTreeIndexes.taskIdentityPresentation(for:)` 取得 O(1) 投影，不在 View 中拆分路径字符串。`.hierarchical` 只显示标题，由现有树缩进/section 提供层级；`.standard` 显示标题与不含自身的父级路径，搜索结果等脱离树的表面必须使用它；`.compact` 才使用完整路径。`TaskSummaryRow` 是 Tasks、Sidebar、层级选择器和 `TaskIdentityRow` 的共享视觉语法：标题优先，正常字号最多两行；`.hierarchical` 的标题后第二行、或 `.standard` 的次级父路径之后，metadata line 按“左侧 checklist progress → flexible spacer → 被动状态组（Running → Selected）→ 已工作时长 → navigation chevron”排列。Running 与 Selected 使用相同紧凑圆形 SF Symbol 槽、字重和光学包络；辅助功能字号可纵向生长。被动 `TaskRunningIndicator` 只说明状态，不执行 Stop；选中状态继续由 selection trait 提供辅助语义，Start/Switch/Stop 命令必须是独立控件，三者都不得替换任务图标。`TaskVisualPresentation` 在进入 SwiftUI 前规范化 symbol 与颜色。
- Task category headers：`TaskCategorySectionHeaderStyle` 明确共享分类 header 的三个展示意图。Tasks 管理页使用跨平台 `.standard` 的 `body.semibold`，并保留完整 action/辅助字号行为；临时层级选择器显式使用 `.compact` 的 caption 密度；regular shell 持久侧边栏显式使用 `.sidebar` 的 `body.semibold`，且 SF Symbol 继承同一系统字体。调用方不得用平台条件、Bool 或硬编码 point size 再次混淆 picker 与 sidebar。
- Task timer actions：`TaskTimerActionButton` 统一 Today、Quick Start 和计时选择器全部 Start/Switch/Stop 的 destructive role、满足 iOS/iPadOS 至少 44 pt 点击目标的尺寸、macOS 原生 28 pt icon-only 尺寸、icon-only/title-and-icon 分支及带任务名的辅助标签。title-and-icon 使用系统 bordered capsule；其 macOS 可见文字使用 `body.semibold` 且标签本身至少 20 pt，不能依靠字体放大伪造 hit target。计时选择器的 icon-only 使用单层圆形 SF Symbol，并固定在 iOS/iPadOS 54×54 pt、macOS 28×28 pt 的操作槽中。Start、Switch、Stop 分别使用 `play.circle.fill`、`arrow.left.arrow.right.circle.fill`、`stop.circle.fill`，共享系统字体配置、可见圆形包络、按压/禁用反馈与尾部位置，不允许逐状态缩放或偏移。它只呈现调用方已经决定的 `TimerPickerSelectionCommand` 或精确 active segment，不拥有准入策略。选择器中的 `TaskSummaryRow` 是独立只读语义元素，动作是独立原生 Button；两者不得嵌套，也不能隐藏摘要后让 Stop 获得整行点击范围。
- Task navigation：`TasksNavigationView` 是 iPhone、iPad 与 macOS 唯一的任务导航容器，store-owned `TasksRoute?` 是页面事实来源。regular sidebar/detail shell 直接以 route 选择 `TasksView` 或 `TaskDetailView`，任务 A → B 是原位替换，不得先清 route 暴露 Tasks 根页；compact shell 才把同一 route 映射为 typed `NavigationStack` path，并把系统 Back 作为受草稿保护的清 route 请求。旧详情的 dismiss closure 必须按 task ID fencing，不能清除后来替换的 route；`TaskDetailView` 的 task-keyed loader 也必须随 route identity 重建初始草稿。`selectedTaskID` 表示计时器等领域使用的业务选择，`tasksRoute` 只表示当前页面；系统 Back 不得清空业务选择。打开详情必须先验证任务存在且未被历史 tombstone 隐藏；归档成功、外部 tombstone、全量维护或刷新发现 route 失效时才清路由，写入失败必须保留原详情与选择。iPad/macOS sidebar 的 selection 直接从 route/destination 派生，并在 task-tree revision 变化时重新展开当前任务的新祖先，不维护第二份镜像 `@State`。
- 普通 Task Detail：只读优先的 `List`，铅笔按钮在同一 workspace 中进入编辑态；清单完成状态可直接切换。系统 navigation title 与首张 identity row 都显示任务标题：前者属于导航 chrome，后者以标题优先、父级/根路径次级的层级承担内容身份。任务运行时，计时状态只由动作区明确的 Stop 表达，不在 identity row 重复 Running；Add Time 等其它明确动作仍按自身语义保留。核心身份/动作/清单/预测/备注不得依赖分析快照；分析尚未完成时仅在其自身 section 显示系统 `ProgressView`，不使用全页 loading replacement。返回完全交给系统导航控件，详情页不得添加自绘 Back 或额外调用 `dismiss()`；归档成功或外部 tombstone 使 route 失效时自然 pop，归档失败停留在原页。
- Apple Health Task Detail：只按 `AppleHealthTaskCatalog.taskRole(for:)` 的 canonical ID 进入只读特例，不得用更宽的 sync-only/不可计时集合代替。列表只组合 Summary、Task Analysis、Recent Records；周期选择和前后周期导航嵌入 Task Analysis。有旧快照的刷新、失败或不可用反馈也在分析区内联；完全没有快照时可以显示单一原生状态区。该路径不显示普通 identity/editor、availability、quantity、heatmap tracking、forecast、Add Time、More/Archive、autosave error 或 draft recovery；普通任务仍走原组合。
- Task editors：任务、分类、checklist 与 Pomodoro 计划的符号/颜色入口共用 `SymbolColorPickerButton`。iOS 入口必须用 `NavigationLink` 推入编辑器已经拥有的 `NavigationStack`，Back 只返回同一草稿，最终 Save/Cancel 仍由外层 sheet 独占；不得在编辑 sheet 内再叠加一个带伪 Done 的 sheet。颜色由共享 `SymbolColorWell` 承担：iOS/iPadOS 在 scene-owned 的系统 SwiftUI popover 中直接复用 BlossomColorPickerCore 的默认 `ExpandedBlossomView`、`PetalLayout`、模型、色板、亮度控制与命中逻辑，并把整个上游视图按 `44 / BlossomConstants.petalSize` 等比放大；不得重新排列花瓣、重建色轮、另写亮度轨道，或在系统容器内再添加应用自有 picker 卡片。入口与放大后的可见色瓣均为 44 pt。macOS 复用相同公开 Core 与默认 194 pt layout，由薄 AppKit presenter 从实际颜色 well 的 `NSView` 经其所属 `NSWindow.convertToScreen` 换算锚点并创建 owner child window；不得使用 `NSApp.keyWindow`、magic offset 或复制花瓣几何。应用适配层只负责 presentation、统一缩放、焦点和六位 sRGB binding。任意有效三位/六位 sRGB 输入都规范化为六位持久值；选中符号、Checklist 完成标记和 Timeline 色块通过 `TaskColorPalette.contrastingForegroundColor` 按实际背景选择黑/白前景。搜索键盘出现时，符号滚动区仍必须在键盘上方至少保留一个完整可点行；打开颜色、选择符号、Return/Done 或滚动会结束搜索焦点。新建任务和分类标题每个编辑会话只自动聚焦一次，从子页面返回时不能重新抢焦点。
...
- Task rows：Tasks 列表不再维护普通、紧凑和 Accessibility 三套视觉 row；它把 canonical identity 与必要 metadata 交给 `TaskSummaryRow`。正常字号优先让标题最多显示两行；树内使用 `.hierarchical` 避免重复路径，搜索/平铺结果使用 `.standard` 保留父级上下文。metadata line 左侧是 checklist，右侧依次是被动 timer 图标、已工作时长和 iOS navigation chevron；Running 不是 Stop，行内不得加入停止命令。`TaskManagementRowAccessibilitySnapshot` 仍是任务详情按钮的单一 VoiceOver 投影，并继续补足视觉摘要未常驻显示的完整路径、预测与子任务数；不要用 `accessibilityRepresentation` 或忽略 children 后只补部分字段。context menu 与 swipe 只复用同一个 Archive 动作；活动 timer/Pomodoro 子树不提供 swipe Archive，并在 context menu 说明必须先停止。Settings 的 Archived Tasks 使用同一任务摘要和 Restore 命令，父任务必须先于后代恢复。普通任务界面、facade、command、use case 和 repository 不得重新引入 Delete 命令链。
- Task availability：`TaskTrackingAvailabilityService` 一次线性扫描先产出可见/父级资格，再结合可见 recurrence rule 与 occurrence 把模板从 direct-work 资格中移除。归档兼容以 `archivedAt != nil` 或历史 `statusRaw == "archived"` 任一成立为准；归档写入继续双写两者，恢复预检从 `LegacyTaskStatusRaw` 接受 V4 的四种旧值，不迁移或批量回写 CloudKit。`planned`、`active`、`completed` raw 只做 round-trip，不能影响可见性、层级、编辑或计时。归档分支与历史/外部 tombstone 分支不可见且不接收新工作；归档活动子树前必须先停止 timer/Pomodoro，历史 segment 编辑可保留原任务。Timer、Pomodoro、手工记录和 App Intent 共用 direct-work 判定；Inbox、父任务选择、清单建议、任务菜单和 Today Heatmap 共用 parent/content 判定，禁止再把两者压成一个含义模糊的集合。
...
`TimerRapidRestartPolicy` 只定义普通 stopwatch 的严格 `< 60s` 时间/source/task 条件；SwiftData 关系与 mutation 由 `SwiftDataTimeTrackingRepository+RapidRestart` 在 store-scoped Start 内处理。续接必须创建新的 active segment ID、复用完整且只有一个可见 segment 的原 session、墓碑旧 segment，并确认 gap 内没有其它可见工作及 session 未被任何 canonical 可见 PomodoroRun 引用。关系查询必须先按物理关系收集 ID，再回取同 ID 的所有副本做全局 LWW，最后复核关系，不能在可变 relationship 上先过滤后局部去重。mutation timestamp 必须至少跨过一个 CloudKit 可保真的毫秒并严格支配已经观察到的 future-skewed session/segment winner；`Double.nextUp` 在同步表示中不够。后续 Stop/Edit/Delete 也必须继续向前推进该逻辑时间，不能写回较早的 wall clock。outcome 同时发送 replacement 的 visible event 和 predecessor 原范围的 history event，使跨午夜 ledger index 精确删除旧 ID。短 gap 明确计入连续时长。不得 reopen 旧 ID、重复查询已有的 active snapshot、为普通 Start/Stop 全量 materialize Pomodoro 历史、按待停止 session 制造 N+1 查询、跳过中间 Manual/Calendar/Pomodoro、在 `replaceAll` 中续接，或在 refresh/import/Analytics 读取时回扫历史。

`TimerStoreScope`、`StoreScopedTimerMutationLock` 与 `StoreScopedTimerMutationTransaction` 是协调器的事务底座。持久 store 使用解析既存祖先符号链接后的 canonical 文件 URL，同一内存 container 在完整生命周期内复用一个显式 UUID；成对的 root/target 先使用同一套纯词法组件规则做边界检查，不能分别调用 `standardizedFileURL`。`realpath(3)` 返回的物理路径也不得再次经过 `standardizedFileURL`，因为 iOS 会把 `/private/var` 反向改写为 `O_NOFOLLOW_ANY` 必须拒绝的 `/var` 系统别名。锁文件与 store 同目录并使用 `.timer-mutations.lock` 后缀。锁实现复用 `PathFileLockRegistry`/`PathProcessFileLock` 的进程内递归锁与跨进程 `flock`，不得另造第二套文件锁。transaction 必须先取得 store lock，再创建新的 `ModelContext`、关闭 autosave，并用一次 `performAtomicMutation` 统一保存或回滚。最外层 save boundary 必须显式设置稳定的 `TimeTrackerHistoryAuthor` 并在 `defer` 中恢复调用方原 author：普通 scene 与 fresh coordinator 写入使用 `.localMutation`，sync restore 使用 `.syncReconciliation`，启动 migration/seed 使用 `.bootstrapMaintenance`；内部 command/repository 的嵌套 atomic primitive 继承外层 author，不能覆盖。`nil`、旧版本或未知 author 均不得被 persistent-history consumer 推断为本地写入。锁内只做授权、fresh fetch、plan、model mutation 和 commit；网络、UI 等待及 post-commit read-model/system-surface refresh 留在锁外。Timer、Pomodoro start/break-resume 与 active Segment 编辑必须用 `SwiftDataTaskRepository.directWorkTask(id:)` 只读取目标、可见祖先链及目标相关 recurrence claim，不得为单一目标 materialize 全部 task/rule/occurrence；其 `allowParallelTimers` 必须由 `TimerAdmissionPreferenceResolver` 在此 fresh context 内只查询对应 key，再由 `AppPreferences` 对同 key 的 active/tombstone 副本做全局 LWW，Watch、App Intent 和 facade 不得传递缓存 Bool。Watch processor 必须把锁内实际 outcome events 与 `forcedSystemSinks: [.watch]` 放入同一个 post-commit receipt，不能从旧 Watch snapshot 猜测停止范围或另造一代。Timer、Pomodoro、手工时间、新建/编辑 segment、task lifecycle/draft、daily recurrence rule/materializer、countdown 新建/更新/删除、同步偏好、同步快照、checklist quick commands 和 Inbox primary commands 已共用此锁域；新增写入口不得退回 scene-owned `ModelContext`。仅设备本地的 LLM Keychain 更新也要在该锁内与完整配置保存串行：读取旧 key、写 key 与 preference commit 同一临界区完成；SwiftData 失败时仍以补偿恢复 key，不能把锁误写成跨存储 ACID。
...
`AnalyticsSelectionPolicy` 固化所有会被 `.first` 或单值摘要消费的决胜规则。Task breakdown 按 gross seconds 降序、wall seconds 降序、本地化标题升序、UUID 升序；并列 peak hour 选择最早的本地小时。任务只有历史 tombstone、当前读模型不可见时，breakdown 与 overlap 共用 session title resolver，按 `startedAt`、`updatedAt`、UUID 选择最新有效 snapshot。不得依赖 `Dictionary(grouping:)` 的遍历顺序，也不得在不同投影重新实现不同 fallback。
...
重复任务运行时由 `StoreScopedTaskRecurrenceCommandCoordinator.swift` 负责锁与公开入口，`+RuleMutations.swift` 负责 immutable identity、启停和活动工作准入，`+Materialization.swift` 负责当前日幂等生成，`TaskRecurrencePersistenceState.swift` 负责 physical claim/tombstone/partial-import 视图，`TaskRecurrenceMutationModels.swift` 负责值类型 outcome。规则时区创建后冻结；materializer 只处理当前日、不回填、暂停日不补，模板不可直接工作但仍可作为父任务。重放必须保留用户对生成任务的编辑；任何物理 claim、墓碑或分阶段导入局部行都会阻止后台修复。Facade 只负责触发与刷新，App 生命周期只负责启动、前台、任务 revision、时钟和下一午夜调度。
...
同一 App 进程中的多 scene 通过 `StoreMutationBroadcaster` 加速收敛：本地 durable commit 完成且发送 Scene 的必要 read models 已刷新后，把 events 放入有界有序队列，并在下一次 MainActor 调度中广播；其它 scene 只按 event plan 刷新 read model，并在任务/ledger plan 时校正失效 selection/route，不记录 snapshot、不发 system surface 或自动启动 LLM。发送者按 identity 跳过重复 refresh。Scene、App Intent 与 Watch 同时把同一 outcome receipt enqueue 到物理 `TimerStoreScope` 共享的 `CommittedMutationSystemProjectionScheduler`；调用方不等待任何 lane。已经开始的副作用不取消，pending generation 合并事件，ack/pending receipt 各最多保留 512 个；超过事件/identity/偏好-key 字节预算时提升为 `.fullSync`，不得无界积累。

`PersistentHistoryLaneCursorStore` 是提交后恢复协议的耐久 frontier：sync snapshot、Widget、Watch 与 Live Activity 各有独立 sidecar，直接 Codable 完整的 opaque `DefaultHistoryToken`，不得读取或自行构造 token 内部值。已成功协调的空 store 使用 `ready(nil)`，与“cursor 缺失”不同。增量推进必须在 sink 成功后对 expected token 做单调 compare-and-swap；另一个进程已经推进到同一或更晚 token 才可视为已确认，任何其他不匹配都必须重试。首次、cursor 损坏、store identity/format 不匹配或 token 过期时，先登记 full-reconciliation attempt 并取得当时 history tail，再发布完整事实，最后以同一 attempt 建立 baseline；attempt 存在期间增量确认必须暂停，较旧或被后续 attempt 取代的 worker 不得覆盖新 cursor。损坏或超限的 attempt 不能等价于“不存在”：隔离 attempt 时必须同时隔离该 lane 的旧 cursor，当前调用按中断的 full reconciliation 处理，后续调用只能看到缺失 baseline。当前 attempt 可以把过期 token 重建为已协调的空 baseline。`PersistentHistoryProjectionDriver` 在专用 actor 上分页读取由独立 `@ModelActor` 隔离的真实 SwiftData history，并在 effect 成功后才确认上述 frontier；in-memory container 只保留同一 driver 生命周期内的 volatile baseline。查询必须只用 `token > cursor`，再逐 transaction 校验物理 store UUID 和 author；不能把 author 过滤进 descriptor 而跳过统一 frontier。所有 author 都必须推进 frontier，但只有精确的 `localMutation` author 能触发 sync snapshot；history change 的 entity name 通过 `PersistentHistoryProjectionImpact` 映射成不带陈旧 ID/range 的保守事件，未知实体必须提升为 `.fullSync`，不能在无法理解影响范围时仍确认 cursor。任何 lane 都无权调用 `deleteHistory`。`forcedSystemSinks` 是当前状态请求，不伪造 history：即使 frontier 已追平或 history 与 Watch 无关，也只执行指定 sink；失败不确认 force，merge/retry/backlog 必须保留它。

`CommittedMutationSystemProjectionWorker` 为 sync snapshot 使用独立 `PersistentHistorySyncSnapshotWorker` actor；三个系统表面则由短生命周期 `CommittedMutationSystemSurfaceMaterializer` 的 fresh `@ModelActor` 一次读取 committed facts并复用纯 DTO。Widget JSON/App Group 写入由 `WidgetSnapshotProjectionWriter` actor 串行，WatchConnectivity/WidgetKit/ActivityKit 只在各框架要求的位置发布。每次读取、缓存、发布与确认前后都检查 scheduler container revision；容器替换会丢弃旧 DTO/ack，但不取消已经开始的 I/O。某一 lane 失败不能阻塞或反转 sibling lane，失败摘要也不能间接持有 `ModelContext`/container。

Cloud recovery 的物理 store reset 在既有 store-scoped mutation lock 内再持有 `DurableLocalFile` root lock，先原子递增保留的 reset epoch，再删除 SQLite、WAL/SHM、四 lane cursor 与 attempt。reset epoch 损坏或超限时，普通读取和注册保持 fail-closed；只有显式物理 reset 可以在同一锁边界中先废弃所有 lane 元数据、隔离坏 epoch，再写入随机的非零新 epoch，从而既恢复可用性又拒绝旧 coordinator 的确认。普通 cursor 调用方通过 `PersistentHistoryLaneCursorStore.registeringCurrentEpoch` 固化注册时 epoch；history driver 还必须按 `epoch-before → Core Data metadata store UUID → epoch-after` 注册，并且只在两次 epoch 相同时一起缓存 UUID/frontier store，避免把被替换 SQLite 的 UUID 绑定到新 epoch。禁止假定初始 epoch 为 0，也不得把 root lock、metadata 或文件 I/O 放在 MainActor，因为这些操作可能等待。begin、incremental CAS 与 full-reconciliation finalize 都重验当前 epoch；任何失败都丢弃 driver 缓存的注册，确保下一次 run 会重新绑定当前 epoch。因而 reset 前挂起的 worker 即使随后恢复，也不能重建旧 cursor；新 container 只能在新 epoch 下重新建立 baseline。reset epoch sidecar 与 mutation lock 本身不随 store basename 清理。
...
普通任务只提供可逆的 Archive/Restore：Archive 隐藏分支但保留历史，活动 Pomodoro 或 timer 必须先由用户停止，命令不会静默终止工作。`TaskNode.deletedAt` 不再对应产品 Delete 动作，但仍是旧客户端、CloudKit/import、权威重置/恢复和 LWW 去重必须理解的 tombstone 协议。普通 Local、iCloud、local-fallback 和 emergency 生产模式没有跨设备删除确认，因此 `AppCloudSync.allowsPermanentTombstonePurge` 为 false，`DatabaseMaintenanceService` 直接返回 0。只有隔离的 Demo/UI Test store 可物理清理过期 tombstone graph。
...
HealthKit 样本只进入 CloudKit-disabled 的 `AppleHealthReplicaSchemaV1` store。`AppleHealthReplicaRepository` 是唯一 SwiftData owner，向 UI/Analytics 返回不可变值快照；增量 apply 只按 changed/deleted UUID 分块查询受影响记录，范围 snapshot 必须把 overlap 条件下推为 SwiftData predicate，不能先抓取全表再在 Swift 过滤。它必须接受 HealthKit 合法的 `startedAt == endedAt` point sample，只有反向区间才拒绝整批提交。point sample 作为来源事实保存在 replica 中，但既有时长投影不会把零宽区间计入时间线或分析。`AppleHealthReplicaSyncService` 在两个 anchored stream 都成功后，才在一个 replica transaction 内提交 upsert、明确删除与双 anchor；普通范围切换只调用 clean-cache 路径，并发调用共享一个 anchored query，单个 waiter 取消不能中止仍有消费者的共享工作。启动需求、前台、显式刷新/重试、授权、Clear All 和 `HKObserverQuery` 事件才把 generation 标为 dirty。Observer/background-delivery 注册由 Store 持有的幂等 setup task 管理；授权完成后立即允许首次 anchored sync 前进，绝不能 `await` 注册回调而把 UI 永久留在 requesting/loading。setup 失败可由下次入口重试，旧 setup completion 不能清除新任务状态。Observer 必须在增量同步完成后调用 HealthKit completion，再推进 store revision 并从本机 replica 重投影；禁止由页面 timer 轮询 HealthKit，也禁止事件处理后紧接着发起第二次相同查询。不要把 replica model 注册到 `TimeTrackerModelRegistry`，也不要扩展 `SyncDataSnapshot`、Cloud conflict、restore 或 fingerprint。时间线隐藏只清空界面投影；Clear All 才清除 replica records 与 anchors。UI test reader 必须实现 change-reading 和 change-observing 协议，让截图经过隔离的内存 replica，而不是绕过本层。
...
首轮 focus 取消是否丢弃短记录，只能读取该 run 当前 `sessionID` 的 canonical segments；禁止为了一个 session materialize 全账本。若 session 缺失则以 run 的 `startedAt` fallback 计算，不能因为查询收窄改变 20% 阈值语义。

从 break 继续下一轮 focus 是一次新的计时准入。`PomodoroCommandHandler` 必须在同一个原子 mutation 内重新读取 LWW 后的 canonical 任务树，并用 `TaskTrackingAvailabilityService` 验证 run 的任务及全部祖先仍可接收工作；归档、历史 tombstone 或缺失任务都返回 canonical no-op。该拒绝发生在 ledger admission 之前，不停止其他 timer、不创建 segment、不推进 run，也不发布刷新或同步事件；不能只信任 facade/UI 可能过期的 `trackableTaskIDs`。
...
通用 ledger 编辑必须保持 Pomodoro 不变量。编辑活动 Pomodoro segment 会重绑 run 的 task/start；把 segment 关闭会按 deadline 完成或取消 run；删除活动 segment 会 tombstone run/session。任务 Archive 在存在活动 timer/Pomodoro 时必须无写入地拒绝；相关 ledger 写入必须在同一个 `performAtomicMutation` 中提交。
...
- `LedgerStore+RecordIndexes.swift` 为每个任务维护按 startedAt/UUID 完全决胜的最近 8 条 segment ID，并在 scoped replacement 时增量更新；只有删除或改期的 ID 本来就在该有界集合中，才从任务完整 ID set 补齐被挤出的第 9 条。集合外的旧记录更新不得排序完整任务历史。Task Detail 的分钟级刷新通过 `segments(overlapping:taskIDs:evaluatedAt:clockReference:)` 直接求当前/上一比较周期与任务分支的交集：planner 先用 task ID bucket 数量和 day/long-span/time-sensitive bucket 上界选择较小的一侧，只物化该侧 ID，再做精确 overlap；不得先物化全 App 周期记录或该任务全部历史。Recent records 另从有界 recent index 合并。若 CloudKit 关系隔离使有界 recent 候选不可见，facade 才为正确性回退到完整 task index，正常一致数据不走该路径。
...
- Analytics snapshot 呈现由 snapshot 与对应 request 组成一个 atomic loaded presentation。精确 `AnalyticsEvaluationCacheKey` 命中可在异步任务启动前同步呈现；没有任何 presentation 的首屏仍可使用原生 loading row。已有 presentation 后切换 range/interval 时，landing 与 category detail 保留稳定 section/card 壳，但数据内容必须 redacted、禁止 hit testing 并从辅助功能树隐藏；分支判断继续使用 loaded request，不能把旧 Today 图表套到 Week/Month 控件下。同一 range/interval 的 revision/live bucket 刷新可继续显示真实 snapshot。刷新 indicator 永久占用固定尺寸布局槽，不得改变 `ViewThatFits` 的 fit 结果；取消或过期任务不得发布中间 snapshot/request 组合。
...
同步刷新是事件驱动的：`NSPersistentStoreRemoteChange` 和 `NSPersistentCloudKitContainer.eventChangedNotification` 进入 `TimeTrackerStore+SyncObservers`。首个通知建立固定 350 ms 截止时间，后续通知只合并进常量空间的 batch，不取消并后推计时器；否则持续导入会让刷新饥饿并让原因队列无界增长。窗口保留最高优先级活动、最新 import 结果、是否需要冲突处理与是否需要 read-model catch-up，再由 refresh planner 至多执行一次一致性刷新。纯 Cloud export completion 只结算 export checkpoint、冲突提示和 typed activity，不重复本地 mutation 已完成的全域 read-model refresh 或四路系统投影；若同一批次还有 remote/import/setup 或 export conflict，则仍 catch up 一次。启动、scene 回到 active 与完成 remote import 在 read-model refresh 后还会 enqueue `.fullSync` projection catch-up；启动额外 force Watch current-state，以恢复进程退出打断的无 history terminal publication。catch-up 不重复自动 suggestion 或当前 scene 副作用；不要重新引入常驻 5 秒轮询。`SyncedPreferenceService.latestByKey` 必须先完成 LWW/tombstone 选择，再过滤已删除结果，否则旧 active preference 会复活。legacy `UserDefaults` 迁移也必须从 logical-key LWW winner 判断 key 是否已迁移；winning tombstone 仍表示“已迁移”，必须阻止旧本机值重新导入。
...
`SyncConflictService.prompt()` 是 throwing read boundary：只有合法 state 中确实没有完整 pending conflict 才返回 `nil`。损坏、超限、权限和文件系统错误必须传播，禁止用 `try?` 转换成“无冲突”。首次 Store 配置必须在挂接 facade repositories 及任何 migration/seed/refresh 前完成该读取；失败时设置现有 recovery safety 与诊断并保持 startup incomplete，不能让 direct command 借已挂接的 repository 绕过门禁。`recordLocalMutation` 仍在同一 state lock 内从刚保存的 runtime state 生成 `.recorded(prompt:)`，而 persistent-history at-least-once 重放相同 snapshot 时比较完整后置条件，不能只比较 fingerprint；已经满足就不得推进 generation 或重写 manifest/slot。sync lane 成功后由 `SyncConflictPromptChangeBroadcaster` 通知所有 scene；每个 Store 使用一个 serialized reader，single-flight 期间最多合并一次 trailing read，100/300 ms 有界退避并在 foreground 强制重读。request ID 与取消检查保证旧结果不能覆盖新状态；读取失败保留最后已知 prompt，不写共享错误，也不改变 Watch/App Intent 终态。显式 conflict resolution 成功必须广播清除，使 sibling scene 不保留旧 prompt。
...
Intent durable mutation 提交后，`SystemActionPostCommitEffects` 把 command 返回的 exact events 同时交给 queued scene broadcaster 与 store-scoped `CommittedMutationSystemProjectionScheduler`，随后即可返回。sync snapshot、Widget、Watch 与 Live Activity 由 persistent-history lane 后台追赶；Intent 不创建临时 facade/context、不等待 projection、不启动完整应用 lifecycle 或自动 LLM。任一 lane 失败只进入该 lane 的 retry/diagnostic，不能把已提交的非幂等动作返回为失败并诱发系统重试。
...
Watch target 的状态 owner 是同一个 `WatchAppStore` 类型，但职责按 extension 文件拆开：base 文件只持有 observable state、依赖和恢复；Commands 文件处理 submit/retry/discard、20 秒确认 timeout 与本机 queue persistence；Connectivity 文件处理 activation/transmit、payload/result/snapshot application 与 freshness/error；SessionDelegate 文件只负责 WCSession callbacks。手机 facade 同样分工：`TimeTrackerStore+WatchSnapshot.swift` 保留纯值投影/预算/兼容 membership oracle，后台 materializer 复用同一排序与树索引服务；`TimeTrackerStore+WatchCommands.swift` 处理命令、相关 read-model refresh 与 scheduler receipt。新增逻辑应进入对应 owner，不能重新把 transport、delegate、queue lifecycle、快照和命令处理混回同一文件。

Watch 使用持久快照加命令队列。每个 `WatchTimerCommand.id` 是幂等键；新命令和进程恢复命令走 durable `transferUserInfo`，可达时再用 `sendMessage` 加速；单纯 reachability 变化只重发即时消息，不能重复制造 durable 副本。界面共有七态 typed terminal result（success、duplicate、missingTask、missingSegment、invalid、failed、timeout）；iPhone 处理结果用 durable user-info 回传且不等待新快照，timeout 则是 Watch 本机等待 20 秒后生成。每个 iPhone `handleWatchCommand` outcome 都 enqueue `forcedSystemSinks: [.watch]`：真实 mutation 同一 receipt 还携带锁内 exact events，duplicate/missing/invalid/failed 等无 mutation 只使用空 events + forced Watch，不触发 sync/Widget/Live Activity；本机 timeout 不经过 iPhone scheduler。timeout 后 retry 保留 ID、刷新 `issuedAt`，用户也可 discard。`WatchCommandProcessor` 在 receipt lookup 后、任何 mutation 前校验 DTO 和时间边界：命令最多保留 30 秒，允许最多 5 分钟的未来设备时钟偏差；过期/非法命令返回 invalid 且不写 receipt 或 ledger，因此用户仍可用同 ID 明确重试。快照反射只为旧手机兼容确认。
...
自动建议是另一个明确的本机同意开关，默认 false，不进入 CloudKit 或 JSON。只完成配置不会开启后台发送；开启后才会为 Inbox/checklist 触发受并发和退避控制的请求。Checklist 失败 fingerprint/retry metadata 只保留当前未删除、未完成且任务仍可用的 item ID；任务/checklist refresh 会裁剪失效 key，关闭或全量取消会释放全部 metadata，禁止长期创建/删除条目导致 scene store 无界增长。发行时必须锁定默认 endpoint/第三方 endpoint 的运营方、用途、保留期、删除渠道和隐私披露；“OpenAI-compatible”不是数据不保留的保证。
...
任务计划的生产链是 `LLMTaskWorkspacePlanningService` → `AITaskWorkspaceOverlay` → `AITaskWorkspacePlanGeneratorViews` → `StoreScopedAITaskAtomicMutationCoordinator` → `TimeTrackerStore+AITaskPlanCommands`。已替换的 `LLMTaskPlanService`、create-only draft/mutation models、`StoreScopedAITaskPlanCommandCoordinator` 和 `saveAITaskPlan` facade 已删除；新代码不得恢复或静默回退到旧 flat JSON。审阅边界按职责拆为 `AITaskWorkspaceReviewPresentation`（稳定行模型）、`AITaskWorkspaceReviewPolicy`（计数、破坏性分类、before/after 映射）和 `AITaskWorkspaceReviewViews`（只读 SwiftUI 渲染）；策略必须保持 Foundation-only 并由行为测试直接保护。
...
多轮 OpenAI-compatible 工具协议通过 `OpenAIChatCompletionModels`、`OpenAIChatToolModels` 和 tool-call assembler 表达 assistant `tool_calls`、匹配的 `role=tool` / `tool_call_id`、finish reason 与 reasoning passback。固定 system contract、Task/Checklist tool description 和内置 mixed few-shot 必须使用同一粒度规则：Task/child Task 是可独立计时的工作单元，Checklist 是一个 Task 内不可独立计时的完成步骤，同一工作不能同时生成两种表示。用户可编辑指令可以补充偏好，但不能覆盖固定规则；历史内置默认只按 exact match 升级，追加过用户文字的版本必须保留。

每个工具 schema 都要求全部声明属性且 `additionalProperties: false`。工具只读或修改纯内存 overlay：list/get；Category reuse/create/update/delete；Task create/update/archive；Checklist create/update/delete；finalize。App 为新实体生成 UUID 并在 tool result 返回，所以 overlay 支持 read-after-write。已有 Task/Checklist 只能按稳定 ID 操作；Category 名称只允许唯一规范化匹配，多个同名必须返回歧义。无效参数（包括目录外图标或颜色）必须保持 overlay 零变更并编码为 `{ok:false}` tool result，让模型在下一轮纠正；未知工具、重复/缺失 call ID、混合 finalize、content-only/create-only 回答以及不支持的 finish reason 仍是显式失败。工具会话不设固定回合数或调用次数，兼容单轮并行与逐轮串行的 provider，只有完成 Task/Checklist 粒度自检并单独调用 `finalize_plan` 才会结束；用户取消、加固传输的 timeout/单响应 2 MiB 边界、provider context 拒绝、工具校验和 proposal-only 原子 Apply 是资源与安全边界，均不得按计划组件数量静默截断。

模型 finalize 后从 baseline→overlay 产生确定性操作 diff。SwiftUI 只读预览显示 create/update/archive/delete/reuse 汇总、完整所属路径与 before→after；`Apply N Changes` 是唯一提交入口，存在删除、归档或进度重置等破坏性影响时使用原生 destructive confirmation。模型工具不能接触 SwiftData。完整 capture 与 Apply 必须 await 专属 `StoreScopedAITaskAtomicMutationCoordinator` actor；不能用继承 MainActor 的 `Task {}`、`MainActor.run`、`Task.detached`、GCD 或 AI 专用重复 repository 规避隔离。actor 在同一 store lock/fresh context 中重新捕获完整 baseline 做保守 CAS，重放并验证被审阅的操作、跨类型/受保护身份、层级、active work 和字段策略，最后只执行一次 atomic mutation；锁内不得出现 `await`。跨 actor 只传现有 Sendable baseline/plan/outcome，ModelContext、SwiftData model 与 context-owned repository 不能越界。任一事实或 revision 变化、任一操作失败或保存失败都整批回滚且不发布事件；stale/失败保留 preview。Task delete 只编译为 Archive，Checklist update/delete 只触碰目标内容或视觉 revision。
...
- [BlossomColorPicker](https://github.com/Lakr233/BlossomColorPicker/) 固定为官方 revision `9a1ee3df309e37ae271362818dcdfdb072ea9611`。它是 MIT 许可、无传递依赖、无网络和数据访问的本地 UI 包；iOS 适配层直接复用其公开 Core，macOS 也复用同一 Core，并用所属颜色 well 的真实 `NSView`/window 完成最薄的坐标与窗口生命周期适配。

依赖 revision/version、`Package.resolved`、[AD-117](AgentDecisions.md) 和 [AD-135](AgentDecisions.md) 必须同步。移除 BlossomColorPicker 时只删除 package 引用与 `SymbolColorWell` 适配，已保存的六位 sRGB 数据不需要迁移。其它依赖仍必须先满足 [AD-011](AgentDecisions.md) 的许可证、供应链、体积、性能、隐私与回退证据要求，不能从这两个例外推导出整套 FlowDown 依赖获批。
