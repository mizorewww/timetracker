...
- Task Detail：canonical router 与 identity/checklist/overview/analytics/navigation/record sections 分文件。`TaskDetailView` 先组合可直接操作的 read-first `List`，再由 request-keyed `.task(id:)` 载入分析快照；`loadedRequest` 必须和当前 request 相等才显示快照，避免范围、任务树或账本 revision 改变时短暂展示旧分析。`AnalyticsRefreshPlan` 只在 active scene 安排下一个分钟/本地日边界，scene 回到前台、时钟或时区变化会重新取样；不得恢复整页 `TimelineView` 或让分析加载遮住计时动作。
