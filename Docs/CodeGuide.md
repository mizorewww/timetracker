...
- Compact shell：实际窗口宽度低于 720 pt 或系统 size class 明确 compact 时，使用五个系统 `Tab`（Today、Inbox、Tasks、Focus、Analytics）。Settings 从 Today 工具栏通过当前 scene 的 `AppPresentationRouter` 打开，关闭后保留原 tab、任务路由与滚动上下文。Tab 切换完全使用系统过渡，不给每页叠加会重复播放的 fade/lift entrance。
- Regular shell：实际窗口宽度达到 720 pt 且 size class 非 compact 时，iPad 与 macOS 共享 `NavigationSplitView` 侧边栏与详情；窗口变窄时可以切到 compact shell，业务 Store、route 和 scene router 位于分支之上。`AppRootView` 的 geometry transform 只输出 `RootLayoutPolicy.WidthBand`，root state 不保留原始窗口宽度。regular detail 只拥有一个外层 `NavigationStack`，`DesktopContentView` 在其中替换 Today/Inbox/Tasks/Focus/Analytics 内容，并在主目的地改变时清空旧页的显式 path；不得为每个主目的地重建 navigation container，也不得让旧页 detail 覆盖新目的地。主目的地是平级关系，只允许短 opacity crossfade，不使用暗示前进方向的 trailing slide；Reduce Motion 时直接替换。不得用设备 idiom、屏幕型号或 `os(...)` 选择这两套产品布局。
- Motion：应用自有时长统一由 `AppMotion` 提供；状态、结构、opacity 和按压反馈不得在 feature 中散落新的 magic duration。披露操作只旋转稳定 identity 的 chevron，不用广域 `withAnimation` 驱动整个列表高度。Reduce Motion 时去掉自定义位移/缩放，最多保留克制的 opacity；滚动、row identity、图表、live resize、Watch/Widget/Live Activity 不增加装饰性动画。
...
- Task timer actions：`TaskTimerActionButton` 统一 Today、Quick Start 和计时选择器全部 Start/Switch/Stop 的 destructive role、满足 iOS/iPadOS 至少 44 pt 点击目标的尺寸、macOS 原生 28 pt icon-only 尺寸、icon-only/title-and-icon 分支及带任务名的辅助标签。title-and-icon 使用系统 bordered capsule；其 macOS 可见文字使用 `body.semibold` 且标签本身至少 20 pt，不能依靠字体放大伪造 hit target。计时选择器的 icon-only 使用单层圆形 SF Symbol，并固定在 iOS/iPadOS 54×54 pt、macOS 28×28 pt 的操作槽中。Start、Switch、Stop 分别使用 `play.circle.fill`、`arrow.left.arrow.right.circle.fill`、`stop.circle.fill`，共享系统字体配置、可见圆形包络、按压/禁用反馈与尾部位置；按压只做轻微局部 render scale，动作状态只做 SF Symbol replace，Reduce Motion 时两者直接切换，不允许调用方逐状态再加缩放或偏移。它只呈现调用方已经决定的 `TimerPickerSelectionCommand` 或精确 active segment，不拥有准入策略。选择器中的 `TaskSummaryRow` 是独立只读语义元素，动作是独立原生 Button；两者不得嵌套，也不能隐藏摘要后让 Stop 获得整行点击范围。
...
同一 App 进程中的多 scene 通过 `StoreMutationBroadcaster` 加速收敛：本地 durable commit 完成且发送 Scene 的必要 read models 已刷新后，把 events 放入有界有序队列，并在下一次 MainActor 调度中广播；其它 scene 只按 event plan 刷新 read model，并在任务/ledger plan 时校正失效 selection/route，不记录 snapshot、不发 system surface 或自动启动 LLM。发送者按 identity 跳过重复 refresh。Scene、App Intent 与 Watch 把 exact events enqueue 到物理 `TimerStoreScope` 共享的 `CommittedMutationSystemProjectionScheduler`，调用方不等待投影 I/O。

投影 request 必须说明 `.localCommit`、`.startupCatchUp` 或 `.surfaceCatchUp`。只有 local commit 和启动补偿记录 sync recovery snapshot；前台、remote import、冲突解决及 forced-only Watch 只重投影系统表面。启动固定发送 `.startupCatchUp + .fullSync`，以幂等完整 snapshot 补偿 commit 后进程退出。scheduler 按 sink 过滤并合并有界事件，单 sink 失败只在下一次相关 generation 重试，不阻塞已经成功的 sibling。

`CommittedMutationSystemProjectionWorker` 为 sync snapshot 使用独立 `PersistentHistorySyncSnapshotWorker` actor；三个系统表面由短生命周期 `CommittedMutationSystemSurfaceMaterializer` 的 fresh `@ModelActor` 每 generation 一次读取 committed facts 并复用纯 DTO。Widget JSON/App Group 写入由 `WidgetSnapshotProjectionWriter` actor 串行，WatchConnectivity/WidgetKit/ActivityKit 只在各框架要求的位置发布。发布前后检查 registry 的 container revision；容器替换会丢弃旧 DTO，但不取消已经开始的 I/O。Cloud recovery 的物理 store reset 继续在 store-scoped mutation lock 和 `DurableLocalFile` root lock 内删除 SQLite/WAL/SHM；投影不再维护平行的 history cursor、attempt 或 reset fence。
