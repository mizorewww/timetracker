...
- Task Detail：只读优先的 `List`，铅笔按钮再打开编辑 sheet；清单完成状态可直接切换。返回完全交给系统导航控件，详情页不得添加自绘 Back 或额外调用 `dismiss()`；删除成功由 route 失效自然 pop，删除失败停留在原页。
