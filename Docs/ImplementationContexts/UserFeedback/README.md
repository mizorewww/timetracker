...
- [x] 01 同一任务小于一分钟的相邻计时段自动合并（[实现上下文](tasks/01-merge-nearby-segments.md)）
- [x] 02 Today 增加本周累计时间柱状图（[实现上下文](tasks/02-today-weekly-time-chart.md)）
- [~] 03 iPad 将正在计时卡片与概览合并成一行（[实现上下文](tasks/03-ipad-now-overview-row.md)）
...
- [ ] 29 主页统计图去除渐变、标题外置并把说明移入信息菜单
- [ ] 30 主页各区块的说明文字统一放入二级信息菜单

## 当前协作

- 主代理：维护总账、Git checkpoint、构建/安装、设备与模拟器资源所有权。
- 当前主产品任务：03 iPad 将正在计时卡片与概览合并成一行。
- 任务 03 已有 `a0837a7` 的候选生产实现；本轮重新审查宽度策略、普通字号 iPad 画面、窄宽回退与 Release 安装后再决定是否勾选。
- 子代理只承担静态审计或明确归属的测试；Simulator、`xcodebuild`、截图与清理由主代理统一分配。
