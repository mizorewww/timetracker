...
- 领域边界与 iPhone、iPad、macOS 普通路径、截图证据已经验收；下一阶段是 Release 全设备安装。
...
- 两台设备只归本任务 UI 验收批次使用；完成后终止 App、关机并删除。
- 既有 `AnalyticsReview-iPhone17Pro` 不属于本任务，不启动、不删除。

## UI 与回归证据

- iPhone 17 Pro / iOS 27：三条人工路径 3/3；结果
  `/tmp/TimeTrackerTask05-iPhone-20260720-10.xcresult`，最终截图在
  `build/Task05InboxShots/iPhone-verified/`。
- iPad Pro 11-inch M4 / iOS 27：三条人工路径 3/3；结果
  `/tmp/TimeTrackerTask05-iPad-20260720-11.xcresult`，最终截图在
  `build/Task05InboxShots/iPad-verified/`。
