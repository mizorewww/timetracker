...
开启功能还会创建一组固定的普通任务模板，包括所有应用支持的运动类型以及“睡觉”。模板集合在读取任何 Health 样本前即已确定，不根据用户是否真实进行过某项运动或睡眠而增减，因此模板的存在不能表达个人健康行为。模板本身可编辑，并按普通任务定义参与 CloudKit 同步；用户对模板的重命名、移动或归档不会被后台刷新覆盖。
