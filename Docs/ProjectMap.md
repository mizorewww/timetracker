...
| `timetracker/Models` | SwiftData models, V1...V13 schema history, migration plan, registry, deterministic recurrence/quantity identities, shared read models, and `TaskEstimatePolicy` | Adding persisted fields, migrations, shared read models, or estimate normalization | Query code, SwiftUI layout, or business workflows |
...
| In-process multi-window read-model convergence | `Services/SystemIntegration/StoreMutationBroadcaster.swift` | `Stores/Facade/TimeTrackerStore+Lifecycle.swift`, `TimeTrackerStore+SyncObservers.swift`, `StoreRefreshPlanner.swift`; persistent history/CloudKit remain the cross-process source of truth |
