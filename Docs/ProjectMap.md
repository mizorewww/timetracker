...
| `timetracker/Services/Tasks` | Task tree validation, canonical metadata repair, title paths, descendants, archive/delete eligibility, legacy status compatibility, flat visible rows, and store-scoped AI plan creation | Changing task nesting, archived/deleted availability, old `statusRaw` handling, orphan/cycle repair, legal parent choices, sidebar/tasks row derivation, or atomic multi-task plan validation | View-local draft state |
...
| Task create/edit/move/archive/delete | `Commands/TaskCommands.swift` | `Models/TaskModels.swift` (`LegacyTaskStatusRaw` compatibility), `Services/Tasks/TaskTrackingAvailabilityService.swift`, the `TaskTree*` service/index/projection family, `Services/Tasks/TaskHierarchyMetadataService.swift`, `Stores/Facade/TimeTrackerStore+TaskCommands.swift`, repository hierarchy/mutation files, `Features/Tasks` |
