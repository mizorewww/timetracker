...
| `timetracker/Services/Inbox` | Inbox suggestion state, logical identity derivation, and store-scoped reorder coordination | Changing when suggestions are shown/hidden/dismissed or how stale cross-scene ordering is rejected | LLM networking or SwiftUI row layout |
...
| Analytics chart data and cache | `Stores/Domains/AnalyticsStore.swift` | `Stores/Facade/TimeTrackerStore+Analytics.swift`, `Stores/Facade/TimeTrackerStore+LedgerRecentRecords.swift`, `Stores/Domains/AnalyticsStore+Caching.swift`, `Stores/Domains/AnalyticsStore+SnapshotBuilding.swift`, `Stores/Domains/AnalyticsStore+TaskSnapshot.swift`, `Stores/Domains/AnalyticsStore+TaskRecentRecords.swift`, `Stores/Domains/LedgerStore+RecordIndexes.swift`, `Stores/Domains/LedgerStore+SegmentQueryIndex.swift`, metrics/breakdown extensions, `AnalyticsStore+Overlap.swift` (facade), `AnalyticsStore+OverlapSweep.swift`, `AnalyticsStore+OverlapMaterialization.swift`, `AnalyticsStore+OverlapParticipants.swift`, `Services/Analytics/AnalyticsEngine.swift`, `Services/Analytics/LedgerBucketCache.swift`, `Features/Analytics` |
