...
| `timetracker/Services/Tasks` | Task tree validation, canonical metadata repair, title paths, descendants, flat visible rows | Changing task nesting, orphan/cycle repair, legal parent choices, or sidebar/tasks row derivation | Persistent task writes |
...
| Task create/edit/move/delete | `Commands/TaskCommands.swift` | `Services/Tasks/TaskTreeServices.swift`, `Services/Tasks/TaskHierarchyMetadataService.swift`, `Repositories/SwiftDataTaskRepository+TaskMutations.swift`, `Repositories/SwiftDataTaskRepository+Hierarchy.swift`, `Features/Tasks` |
...
| Forecast math / incremental rollup | `Services/Forecasting/TaskRollupService.swift` | `Stores/Domains/RollupIncrementalIndex.swift`, `Stores/Domains/RollupIncrementalIndex+Pace.swift`, `Stores/Domains/RollupIncrementalIndex+Activity.swift`, `Services/Forecasting/TaskRollupCalculationContext.swift`, `Services/Forecasting/ForecastDisplayService.swift` |
...
| Watch timer command handoff | `Services/SystemIntegration/WatchCommandProcessor.swift` | `Shared/WatchCommandModels.swift` (queue/result state), `timetrackerWatchApp/WatchAppStore.swift` (persistence/20 s timeout), `Stores/Facade/TimeTrackerStore+WatchSnapshot.swift`, payload codec/bridge, timer commands |
