...
| `timetracker/Services/Tasks` | Task tree validation, canonical metadata repair, title paths, descendants, visible-vs-trackable eligibility, flat visible rows, and store-scoped AI plan creation | Changing task nesting, completed/archived availability, orphan/cycle repair, legal parent choices, sidebar/tasks row derivation, or atomic multi-task plan validation | View-local draft state |
