...
| `timetracker/App/RootViews` | Platform root views: iPhone tab navigation safety, iOS/iPad root composition, and macOS desktop root | Changing per-platform scene root composition or tab navigation safety | Cross-platform domain logic or feature sections |
...
| `timetracker/Stores/Facade` | `TimeTrackerStore`, first configuration, refresh/mutation lifecycle, direct-work/parent eligibility, recurrence commands, and UI-facing facade extensions | Wiring a view action to a command handler, exposing read models, attaching repositories for a committed system action, or coordinating app lifecycle | Domain-sized refresh internals or pure algorithms |
...
| `timetracker/Services/LLM` | OpenAI-compatible endpoint validation, credential-safe transport, response decoding, suggestion services, and bounded task-plan generation | Changing model discovery, redirect policy, concurrency/backoff, suggestion/task-plan prompt contracts, or response decoding | Keychain persistence or SwiftUI presentation state |
...
| `timetracker/Services/Preferences` | Store-scoped synced preference writes and device-only credential coupling | Changing preference commit serialization or Keychain/persisted-configuration ordering | Preference JSON normalization or Settings view layout |
...
| `timetracker/Services/SystemIntegration` | Sync-conflict orchestration/state/export, record-level LWW auto-merge, versioned data snapshots, durable local-file/lock primitives, credentials, App Group/cache DTOs, Watch command processing, and WatchConnectivity payload/transport | Changing conflict recovery, automatic conflict merging, local recovery files, export/snapshot mappings, Widget-visible state, Watch command idempotency, or extension handoff data | Extension UI or durable ledger writes |
| `timetracker/Services/Tasks` | Task tree validation, canonical metadata repair, title paths, descendants, direct-work/parent eligibility, recurrence rule/materialization safety, archive/restore, legacy tombstone compatibility, flat visible rows, and store-scoped AI plan creation | Changing task nesting, recurrence current-day generation, archived or historical-tombstone availability, old `statusRaw` handling, orphan/cycle repair, legal parent choices, sidebar/tasks row derivation, or atomic multi-task plan validation | View-local draft state |
...
| --- | --- | --- |
| Start/stop timer behavior, including rapid restart | `Services/TimeTracking/TimerAdmissionPolicy.swift` and `TimerRapidRestartPolicy.swift`, then `Services/TimeTracking/StoreScopedTimerCommandCoordinator.swift` | `Repositories/SwiftDataTimeTrackingRepository+RapidRestart.swift`, `SwiftDataTimeTrackingRepository+Mutations.swift`, `SwiftDataTimeTrackingRepository+SegmentStop.swift`, `Models/PersistentModelDeduplication.swift`, `Services/SystemIntegration/SyncDataSnapshot+RestoreLedger.swift`, `Commands/TimerCommands.swift`, `TimerAdmissionModels.swift`, `Stores/Domains/LedgerStore.swift`, `Services/Ledger/TimerCommand.swift`, Pomodoro replacement semantics, SystemAction/Watch/AppIntent/deep-link writers, and coordinator lock/fresh-context tests |
...
| In-process multi-window read-model convergence | `Services/SystemIntegration/StoreMutationBroadcaster.swift` | `Stores/Facade/TimeTrackerStore+Lifecycle.swift`, `TimeTrackerStore+SyncObservers.swift`, `Stores/Refresh/StoreRefreshPlanning.swift`, `Stores/Refresh/StoreRefreshCoordinator.swift`; persistent history/CloudKit remain the cross-process source of truth |
...
| AI category/task/checklist plan generation | `Services/LLM/LLMTaskPlanService.swift` | `Features/Tasks/Generation/AITaskPlanGeneratorViews.swift`, `Services/Tasks/StoreScopedAITaskPlanCommandCoordinator.swift`, `Stores/Facade/TimeTrackerStore+AITaskPlanCommands.swift`, task-plan instruction preference/editor, generation/mutation/UI-contract tests, privacy documentation |
| Performance signposts and profiling | `Services/Instrumentation/PerformanceSignpost.swift` | `CorePerformanceBudgetTests`, `Docs/Testing.md`, frozen-source trace evidence in git history |
...
| Widget/Watch snapshot projection or limits | `Services/SystemIntegration/WidgetSnapshotCache.swift` | `Shared/WidgetSnapshotModels.swift` (Unicode-safe shaping/validation), `Shared/WidgetSnapshotStore.swift` (256 KiB App Group boundary), `Stores/Facade/TimeTrackerStore+WidgetSnapshot.swift`, `Stores/Facade/TimeTrackerStore+WatchSnapshot.swift` (running → new pins → legacy non-running Quick 4 → usage remainder membership, then legacy wire reordering), `Stores/Facade/TimeTrackerStore+TaskRanking.swift` (segment-count/last-start/UUID order), `Shared/WatchStateSnapshotModels.swift` (128 KiB text/64 active/256 `recentTasks`, `legacyQuickStartTaskLimit`, legacy pinned-first wire order plus optional `quickStartRank`/`allTasksRank` compatibility), extension targets and snapshot tests |
...
| Watch timer command handoff | `Services/SystemIntegration/WatchCommandProcessor.swift` | `App/WatchCommandRouter.swift` (weak multi-scene dispatch), `Shared/WatchCommandModels.swift` (queue/result state), `timetrackerWatchApp/WatchAppStore.swift` (observable state/restore), `WatchAppStore+Commands.swift` (enqueue/retry/discard/20 s timeout/persistence), `WatchAppStore+Connectivity.swift` (activation/transport/payload/freshness), `WatchAppStore+SessionDelegate.swift` (WCSession callbacks), `Stores/Facade/TimeTrackerStore+WatchCommands.swift` (facade handling/read-model refresh), `Stores/Facade/TimeTrackerStore+WatchSnapshot.swift` (projection), payload codec/bridge, timer commands |
