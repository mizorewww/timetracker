...
- [ ] Split files over the existing size budget where a split can be behavior-preserving: `AnalyticsViews.swift`, `PhoneChromeViews.swift`, `TaskDetailEditorViews.swift`, and `TasksViews.swift`. Progress: `SettingsSectionsViews.swift` has been split into display, Pomodoro, Pomodoro minute picker, countdown, sync, and data-section files with a source-layout budget test. `TaskDetailView.swift` has been reduced to orchestration, with header, analysis, and recent-record sections split into focused files.
