...
- Schema compatibility: current registry is V10. A real V8 disk store containing a legacy `DailySummary` opens through V8→V9→V10 with user facts intact and without that derived cache in the active schema. A real V9 disk store proves Inbox context/revision UUIDs and legacy dismissal state survive V9→V10. Keep the V4 category migration fixture and add further historical fixtures according to risk.
- Inbox suggestion identity: cover dismissal across different physical row UUIDs, same-title unrelated items, title-revision rotation, delete/apply/discard sibling cleanup, stale suggestion matching, logical last-write-wins delete/restore, old snapshot fields omitted, and current snapshot round-trip. Identity must remain opaque UUID metadata rather than a title-derived hash, and storage must stay constant per record.
