...
- Task lifecycle compatibility: archive is true when either `archivedAt` or legacy raw `archived` is present; archive writes both markers, hides the branch, rejects new work, and requires active timer/Pomodoro work in the subtree to stop first. Deleted branches and tombstones keep their existing behavior. Legacy `planned`, `active`, and `completed` raw values remain accepted/round-trippable but behave like ordinary tasks in Today, Quick Start, Pomodoro, manual entry, Inbox suggestion, App Intent, create/move destinations, and task actions. UI contracts must prove there is no workflow-status picker, badge, Complete action, Reopen action, or completed-task blocker.
- Timer start and stop semantics.
- Pomodoro persisted-phase deadline reconciliation: background/startup clipping, idempotency, explicit break continuation, generic timer stop, active segment edit/delete, and task-tree deletion consistency. Break continuation must re-evaluate canonical task-tree admission inside the mutation; archived, deleted, missing, or archive/delete-ancestor-blocked tasks are exact no-ops that preserve the break run and every unrelated active timer without refresh/sync events.
...
- Inbox native-card presentation: launch the dedicated suggestion fixture with `replaceOnLaunch`, then verify capture/open/completed are native List cells with inset margins, the generated icon/color preview and target name are visible, compact layouts retain a readable target width, every compact text action owns at least a 44 pt hit target, and applying the suggestion removes the exact Inbox item. Keep real iPhone and iPad Dark Mode operation-path screenshots plus capture and shared task-picker regressions; source contracts must also preserve unavailable-target, failure, Reduce Motion, and leaf-identifier behavior.
...
- Live Activity presentation: Lock Screen and expanded Dynamic Island reuse one timer row; the fresh expanded normal-size hierarchy contains only task icon, title, and elapsed time in one row, with a stale-state glyph as the sole status exception; tapping opens Today; no Live Activity stop intent, extra-timer badge, or app-opening stop copy remains. Compact and minimal elapsed text must still freeze at the shared stale boundary, and projected identity text must stay below its Unicode-safe payload limits. `LiveActivitySystemSurfaceUITests` is a screenshot slice for explicitly audited Dynamic Island simulator models and skips other destinations rather than treating an iPad or notch-only iPhone as a layout failure.
