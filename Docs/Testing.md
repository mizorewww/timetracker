...
- Synced user preferences, including legacy UserDefaults import. A logical-key LWW tombstone counts as already migrated and must block stale UserDefaults from resurrecting that preference. Use a real read-only disk store to prove failed ordinary migration rolls back pending records, leaves its completion flag false, and preserves the source value; failed secret redaction must leave the SwiftData row unchanged while retaining the safe Keychain copy. Test iCloud enablement separately as device-local startup state, including next-launch behavior and filtering of the historical synced key.
