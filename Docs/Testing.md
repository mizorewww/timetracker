...
TIMETRACKER_LIVE_LLM_SCENARIO=semantics make test-llm-live
...
`semantics` starts with one existing root Task, requires two independently timed
child Tasks plus five untimed Checklist steps under their exact owners, rejects
cross-kind substitutions, applies the production operations, and re-reads the
persisted hierarchy.
...
- macOS Blossom anchor: open the ordinary Task Detail icon editor, record the `symbol.picker.color.well` frame before activation, identify the newly created approximately 194×194 pt window, and assert their centers differ by no more than 4 pt at a location that does not require screen-edge clamping. Retain a normal-size full-screen screenshot proving the SF Symbols popover remains intact and Blossom blooms from its well; the result summary must contain no SwiftUI runtime warning. The existing iPhone and iPad symbol-picker UI test protects their unchanged scene-owned popover branch.
...
- Apple Health replica: verify the independent V1 store reopens on disk and remains outside the main/CloudKit model configuration. Cover first anchored import, idempotent replay, sample modification, explicit deletion, two-stream failure/cancellation/stale-request rollback, atomic anchor advancement, immutable range snapshots, hide-versus-Clear-All behavior, and timeline/task analytics reading the replica instead of direct range queries. UI fixtures must implement `AppleHealthReplicaChangeReading` and use an isolated in-memory replica.
- User JSON export: decode the versioned `timetracker.userData` envelope and assert filtered business data plus deterministic Health workout/sleep records, source fields, record count, and last successful sync time. An empty replica must encode explicit empty arrays; opaque anchors must be absent; an unreadable replica or encoding failure must fail closed without opening the exporter. Keep Cloud conflict export/restore tests on the unchanged business-snapshot format.
...
- Shared segment editor routing: exact namespaced `.trackedSegment` identity must resolve the current canonical draft; an Apple Health identity with the same UUID, a newer tombstone, missing relations, or a busy presentation slot must not open or replace a sheet. Normal-size XCUITests open `segmentEditor.view` from Analytics and ordinary Task Detail, prove save/delete refresh the exact row, and assert Health history remains a non-button. Keep visually reviewed iPhone/iPad editor screenshots and a signed macOS build; if a toolchain-owned crash reporter blocks macOS XCUITest interaction, retain the failing xcresult and report that platform run as inconclusive rather than weakening the production or test UI.
...
- Direct list reordering: normal-size iPhone UI coverage must prove Task checklist and Inbox no longer expose a prerequisite Sort/Edit mode, while behavior coverage moves a checklist draft through the production session/save path and verifies the order in a fresh context. Keep checklist completion-group rejection, Inbox capture/completed exclusion, store-scoped stale-baseline tests, and the independent category-ordering sheet regression.
- Checklist secondary delete actions: behavior coverage must delete by stable draft UUID through `TaskEditorSession`, save through the production editor path, and prove a fresh `ModelContext` cannot reload the removed row. Normal-size iPhone UI coverage proves the permanent trash button is absent, both the visible More-menu delete and trailing swipe delete work, and full-swipe is not required; macOS coverage proves the More menu plus row context menu expose the same destructive command. Keep visually reviewed menu/swipe/context-menu screenshots and leaf identifiers so outer row semantics do not hide the actionable controls.
- Checklist AI visual stability: unchanged task-detail autosaves must preserve checklist item/visual revisions, identical visual writes must be no-ops, and a deterministic fake transport must prove 350 ms latest-input debounce, superseded pending/in-flight cancellation, complete task title/path request identity, and stale completion rejection. `TaskEditorSession` coverage must rebase a visual-only store revision while preserving visible row UUIDs, text, order and dirty state, let local manual visual edits win, and save successfully afterward. The iPhone XCUITest types again without retapping after the first AI result, asserts the software keyboard remains visible, accepts only the final title's color, and retains a normal-size screenshot; the software-keyboard assertion is intentionally iPhone-only.
...
- AI tool protocol and overlay: use deterministic local tests for strict schemas (`required` plus `additionalProperties: false`), assembler mechanics, pure overlay CRUD/read-after-write, App-generated IDs, unique normalized Category reuse, same-name ambiguity, inert prompt-shaped data, cross-kind identity conflicts, orphan/cycle/child-category rejection, depth six acceptance plus depth seven rejection before writes, Task Archive-only, recoverable zero-mutation results for invalid model arguments, and absence of count-based truncation. Decode an actual outgoing task-planning request to prove the fixed contract, editable default, and create/update/finalize tool descriptions all distinguish independently timed Tasks from untimed Checklist steps. Prebuilt provider responses, tool-call sequences, or plans are not model-generation acceptance. `make test-llm-live` must read a short-lived key from the local `.env`/environment and use the three production services against the real endpoint for Inbox, Checklist, prompt28, prompt150, and the mixed Task/Checklist semantics scenario at `max`; prompt150 and semantics must Apply through the production coordinator and re-read their persisted structures. Never print the key or source it from feedback documentation.
...
- macOS typography acceptance: at normal text size, run the deterministic
  `testUIRefactorBaselineScreenshots()` matrix across Today, Inbox, Tasks, Task Detail,
  Focus, Analytics, and the real Settings window. The fixture must replace demo data on
  launch, name attachments with the platform prefix, and capture the actual Settings
  scene instead of choosing the largest app window. Visually confirm that primary
  identity, answers, explanations, actionable status/warnings, and labeled actions use
  native `body/callout` hierarchy, while paths, timestamps, badges, counts, chart
...
Wide Today current-state layout tests use `home.activeTimers.title`, `home.overview.header.title`, `home.now.card`, and `home.overview.card` as leaf/container probes. With deterministic demo data and a valid visible frame, macOS and landscape iPad must place the two visible title tops, the two first-card tops, and the two card bottoms within 2 pt of each other while preserving separate columns. The bottom assertion guards against the shorter Overview leaving a page-background gutter under Gross/Wall when Now owns the row height. Do not compare a title leaf to an outer section group or relax the tolerance to hide a real header-slot mismatch. The iPhone compact regression remains single-column and is not subject to the wide geometry assertion.
