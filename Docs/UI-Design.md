...
- Checklist rows should behave like native to-do rows: a large circular check button, at least 44 pt row height on touch platforms, unfinished items first, completed items after them with strikethrough text. Every task-checklist control (completion, symbol, and actions) centers against the complete wrapped title block, not its first line. Task-checklist titles grow without an arbitrary line cap; Inbox keeps its separately reviewed compact bound. Adding a checklist item should create a focused empty row immediately. Checklist completion is the task's only product-level completion/progress signal and never disables later work.
...
iPhone Today is a native priority-ordered `List` (Now, Overview, Weekly Time, Activity Heatmaps, Quick Start, Timeline, Forecast, Countdown). Use the native large title, keep the gross/wall summary compact without decorative progress or duplicate metrics, and use system buttons for Start Timer and New Task. The global timer-picker launcher is one shared native leading-aligned row across compact and regular layouts: empty Start Timer, parallel Start Another Timer, and exclusive Switch Timer change only their accurate mode-derived title/icon, never prominence or component structure. Empty Quick Start reuses that launcher instead of copying a second button. Quick Start edits through a native sheet/list, not custom chips.

Today section hierarchy has one owner. Overview, Weekly Time, and Activity Heatmaps use `HomeSectionHeader`: card layouts render the title with system `.headline`; iPhone list layouts inherit the native `Section` header typography. Optional aggregate values use secondary monospaced caption text, and the Info button remains a separate trailing control. Weekly Time uses a common zero baseline and side-by-side Gross/Wall bars: Gross stays left in system green, Wall stays right in system blue, and the bottom-leading text legend keeps the comparison understandable without color alone. On compact layouts the chart receives 190 pt so the legend never crowds the plot; regular cards remain 210 pt. In the wide iPad/macOS current-state row, Now and Overview reserve the same platform minimum-interaction-height header slot so their visible titles and first card tops align even when only Overview has a trailing Info button. The row takes its ideal height from the taller card, both cards share that bottom boundary, and Overview centers its two metric rows in the offered card height; do not fake either alignment with an invisible button, measured minimum height, magic padding, or a per-device offset. Do not add a local title font in an individual chart.

On iPhone, visualization backgrounds align to the same inset-grouped row boundaries as Overview and the other native sections. Their content keeps one 16 pt inset inside that boundary. Weekly Time and every task heatmap remain independent rounded cards with at least 10 pt visual separation because iOS can merge consecutive native sections; they must not become one shared outer card or cards inside cards. Keep the system section row transparent, use the shared self-drawn card background, and expand that background from the native content column to the grouped-card edge. The section title stays on the native content column. On iPad and macOS, Weekly Time and Activity Heatmaps form one leading-aligned visualization group. Below 1000 pt of actual content width, that group, Quick Start, Timeline, Forecast, and Countdown stay in reading-order single column. At 1000 pt and above, the visualization group uses a 678...748 pt leading column while Quick Start consumes the remaining 300...410 pt trailing column; the following row gives Timeline the flexible leading column and, when present, keeps Forecast/Countdown in a 360 pt trailing column. The chart region therefore remains bounded at the 720 pt chart width plus two 14 pt card insets without leaving the rest of a wide canvas blank or allowing auxiliary content to change chart geometry. Heatmap tiles use the measured horizontal-scroll viewport rather than a device class: choose the largest whole-point tile from 12...24 pt that fits the selected 5/14/27/53-week range, with 2/3/4 pt gaps as tiles grow. Never shrink below 12 pt to force a long range into the card; overflow initially reveals the newest dates and stays horizontally scrollable. A range that fits disables its local horizontal scroll and remains leading-aligned after reaching the 24 pt readability cap. Month labels use native collision resolution, while the range and intensity legend stay outside the scrolling plot. Do not introduce masonry, a third column, or a custom layout library for this hierarchy.
...
`NavigationStack` + `Form` with `LabeledContent`, native `Picker`, `TextField`, `Toggle`, and toolbar save/cancel. The task estimate control stays explicit: 15-minute steps, zero as "not set," and a concise explanation that it estimates this task's own work while child forecasts remain separate. Reuse `SymbolColorPickerRow` for tasks, categories, checklist items, and Inbox suggestion visuals. On macOS, Blossom must bloom from the center of the actual color well even when that well is inside the SF Symbols popover; only screen-edge clamping may move it away from that center. Do not position it through the app key window or a fixed offset. Checklist rows reuse one shared row component and reorder directly by long-press drag without a separate Sort mode or custom up/down buttons. Movement stays inside the incomplete or completed group. Destructive delete has no permanent trash or More button: iOS/iPadOS expose one leading-edge swipe action with full-swipe disabled, so a rightward swipe reveals Delete in left-to-right layouts; touch long press and macOS right-click expose the same command through a native context menu. Do not duplicate the action on both swipe edges. Category ordering and Quick Start pinned ordering follow the same native-drag/no-arrow rule. An automatic checklist visual update is secondary decoration: merge icon/color into the existing row instead of replacing the draft or text field, preserve the insertion point and software keyboard, and let a local manual icon/color choice win. Acceptance: long checklist text grows beyond four lines without scrolling inside the field, and its completion/symbol controls remain centered against the entire text block; Return submits where the user expects submit; adding a checklist item creates a focused empty row; direct reordering and stable-identity deletion persist after the draft saves; AI visual completion never interrupts continued typing or changes user text; normal-size iPhone, iPad, and macOS screenshots show uncrowded rows plus the native delete gestures; forecast source copy distinguishes explicit estimate from checklist evidence and stays readable at accessibility sizes.

### Analytics

Keep chart math in services and render `AnalyticsSnapshot`. Wrap chart sections in native section-like containers with consistent headers; use Swift Charts where it fits and custom drawing only for timeline and stacked activity; legends are native rows with swatches and labels. Today distribution shows task colors and does not collapse short tasks into one-pixel lines. Acceptance: Today/Week/Month share data semantics; empty states explain what data is missing; long task names do not overlap charts; the landing page keeps a small summary plus category navigation, and at accessibility sizes the range picker may become a menu instead of a clipped segmented control. Activity Heatmaps appear as one discoverable landing row and open a standalone native page; the landing page never expands the full Heatmap. That page keeps the Settings-owned Heatmap range, does not repeat the Analytics date filter, reuses the shared task Heatmap cards, and shows a native configuration empty state when no task is selected. After Analytics has displayed once, a cold Day/Week/Month switch keeps the period controls and section/card shells at stable positions while data rows use non-interactive system redaction; it never substitutes a large blank loading card or exposes old metrics under the new selection. The lightweight spinner owns a fixed layout slot so adaptive control layout does not oscillate during refresh.
...
Let `NavigationSplitView` own sidebar visibility; do not duplicate the system sidebar toggle. Task Detail is the single canonical selected-task surface; do not add a second inspector that diverges from it without a product need. Sidebar task rows are simple navigation rows, not mini task editors. Acceptance: collapsing the sidebar always leaves a native way to reopen it; collapsing and restoring preserves the current detail; selecting task B while task A is open replaces the highlight and detail in one activation without exposing the Tasks root; a dirty A draft remains selected when discard is cancelled and changes to B only after discard is confirmed.
