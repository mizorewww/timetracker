...
`NavigationStack` + `Form` with `LabeledContent`, native `Picker`, `TextField`, `Toggle`, and toolbar save/cancel. The task estimate control stays explicit: 15-minute steps, zero as "not set," and a concise explanation that it estimates this task's own work while child forecasts remain separate. Reuse `SymbolColorPickerRow` for tasks, categories, checklist items, and Inbox suggestion visuals. On macOS, Blossom must bloom from the center of the actual color well even when that well is inside the SF Symbols popover; only screen-edge clamping may move it away from that center. Do not position it through the app key window or a fixed offset. Checklist rows reuse one shared row component and reorder directly by long-press drag without a separate Sort mode. iOS and iPadOS use native move; macOS keeps direct drag plus always-visible up/down controls as the non-drag alternative. Movement stays inside the incomplete or completed group. Destructive delete is not a permanent first-level trash button: every platform keeps a visible More menu, iOS/iPadOS also expose a trailing native swipe action with full-swipe disabled, and macOS adds a native context-menu alternative. Use only the semantic trailing swipe edge so RTL mirrors naturally; do not duplicate delete on both sides. An automatic checklist visual update is secondary decoration: merge icon/color into the existing row instead of replacing the draft or text field, preserve the insertion point and software keyboard, and let a local manual icon/color choice win. Acceptance: long checklist text grows beyond four lines without scrolling inside the field, and its completion/symbol/actions controls remain centered against the entire text block; Return submits where the user expects submit; adding a checklist item creates a focused empty row; direct reordering and stable-identity deletion persist after the draft saves; AI visual completion never interrupts continued typing or changes user text; normal-size iPhone, iPad, and macOS screenshots show the secondary actions without crowding the editable row; forecast source copy distinguishes explicit estimate from checklist evidence and stays readable at accessibility sizes.
