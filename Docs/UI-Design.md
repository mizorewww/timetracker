...
iPhone Today is a native priority-ordered `List` (Now, Overview, Weekly Time, Activity Heatmaps, Quick Start, Timeline, Forecast, Countdown). Use the native large title, keep the gross/wall summary compact without decorative progress or duplicate metrics, and use system buttons for Start Timer and New Task. Quick Start edits through a native sheet/list, not custom chips.
