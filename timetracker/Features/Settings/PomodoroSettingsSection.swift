...
struct PomodoroSettingsSection: View {
    ...
    @State private var pendingDeletionPlanID: UUID?
    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Button(AppStrings.localized("pomodoro.deletePlan"), role: .destructive) {
                guard let pendingDeletionPlanID else { return }
                plans.wrappedValue.removeAll { $0.id == pendingDeletionPlanID }
                self.pendingDeletionPlanID = nil
            }
            Button(AppStrings.cancel, role: .cancel) {}
        } message: {
            Text(.app("pomodoro.deletePlan.confirm.message"))
        }
    }
