...
extension TaskTreeService {
    func categorySections(rootTasks: [TaskNode], categories: [TaskCategory]) -> [TaskTreeCategorySectionModel] {
        ...
        let rootTasksByCategory = Dictionary(grouping: rootTasks) { task -> UUID? in
            guard let categoryID = task.categoryID, categoryByID[categoryID] != nil else { return nil }
            return categoryID
        }
