...
struct CoreArchitectureBehaviorTests {
    ...
    func analyticsLoadsVersionedSnapshotsOutsideTheViewBody() throws {
        ...
        #expect(taskDetailSource.contains("TimelineView(.periodic(from: .now, by: 60))"))
        #expect(taskDetailSource.contains("if let snapshot {"))
        #expect(taskDetailSource.contains("loadedRequest == request") == false)
