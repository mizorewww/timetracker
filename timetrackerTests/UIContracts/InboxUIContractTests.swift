...
struct InboxUIContractTests {
    ...
    func inboxUsesOwnDestinationAndSmoothInlineCapture() throws {
        ...
        #expect(inboxSource.contains(".navigationBarTitleDisplayMode(isCompact ? .inline : .large)"))
        #expect(inboxSource.contains(".navigationBarTitleDisplayMode(isCompact ? .large : .inline)") == false)
