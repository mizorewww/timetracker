...
struct PlatformShellContractTests {
    ...
    func sidebarUsesStableRowsAndTracksProgrammaticNavigation() throws {
        ...
        #expect(source.contains(".onChange(of: store.desktopDestination)"))
        #expect(source.contains(".onChange(of: store.tasks.map(\\.id))"))
        #expect(source.contains("store.task(for: desktopTaskDetailID) != nil"))
