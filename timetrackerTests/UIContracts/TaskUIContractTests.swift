...
struct TaskUIContractTests {
    ...
    func phoneTabsBindToSharedNavigationDestination() throws {
        ...
        #expect(rootSource.contains("PhoneDestinationDeck(store: store, selectedDestination: chrome.selectedDestination)"))
        #expect(rootSource.contains("@State private var cachedDestinations"))
        #expect(rootSource.contains("store.prewarmDestinationCache(for: destination)"))
        #expect(rootSource.contains("AnalyticsView(store: store, isActive: isActive)"))
        #expect(rootSource.contains(".allowsHitTesting(destination == selectedDestination)"))
        #expect(rootSource.contains(".accessibilityHidden(destination != selectedDestination)"))
        #expect(rootSource.contains("Task.sleep(for: .milliseconds(140))"))
